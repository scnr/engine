...
class SCNR::Engine::Plugins::EmailNotify < SCNR::Engine::Plugin::Base
    ...
    def clean_up
        ...
        if options[:report] == 'ser'
            opts[:attachments] = {
                "report.#{Report::EXTENSION}" => framework.report.to_ser
            }
    ...
    def self.info
        {
            ...
            options:     [
                ...
                Options::MultipleChoice.new( :report,
                    ...
                    default:     'ser',
                    choices:     ['xml', 'html', 'json', 'yaml', 'marshal', 'ser', 'none']
                )
            ]
