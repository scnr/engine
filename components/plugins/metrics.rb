...
class SCNR::Engine::Plugins::Metrics < SCNR::Engine::Plugin::Base
    ...
    def run
        ...
        http.on_complete do |response|
            response.platforms.to_a.each do |platform|
                @metrics['platforms'][SCNR::Engine::Platform::Manager.find_type( platform ).to_s] << platform.to_s
            end
        ...
        framework.on_page_audit do |page|
            %w(links forms cookies headers jsons xmls).each do |type|
                page.send( type ).each do |e|
                    ...
                    if e.is_a? SCNR::Engine::Element::Form
        ...
        metrics['scan']['authenticated'] = !!SCNR::Engine::Options.session.check_url
