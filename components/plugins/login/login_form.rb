...
# @version 0.2.1
class SCNR::Engine::Plugins::LoginForm < SCNR::Engine::Plugin::Base
    ...
    def prepare
        ...
        response = begin
            session.login( true )
        rescue SCNR::Engine::Session::Error::FormNotFound
            msg = STATUSES[:form_not_found]
            if (form = session.find_login_form( url: @url ))
                inputs = form.inputs.keys.select do |n|
                    ![:hidden, :button, :submit].include? form.field_type_for( n )
                end
                msg += " Found form with inputs: #{inputs.join( ', ' )}"
            end
            ...
            return clean_up
        rescue SCNR::Engine::Session::Error::FormNotVisible
            register_results(
