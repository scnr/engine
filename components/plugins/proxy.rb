...
# data to {SCNR::Engine::Framework#push_to_page_queue} to be audited.
#
# @author Tasos "Zapotek" Laskos <tasos.laskos@gmail.com>
class SCNR::Engine::Plugins::Proxy < SCNR::Engine::Plugin::Base

    BASEDIR  = "#{File.dirname( __FILE__ )}/proxy/"
    BASE_URL = 'http://scnr.engine.proxy/'

    MSG_SHUTDOWN = 'Shutting down the SCNR::Engine proxy plug-in...'

    MSG_DISALLOWED = 'You can\'t access this resource via the SCNR::Engine ' +
    ...
    def run
        ...
        print_info
        print_info 'When browsing HTTPS sites, please accept the SCNR::Engine SSL certificate' +
            ' or install the CA certificate manually from:'
        print_info "    #{SCNR::Engine::HTTP::ProxyServer::SSLInterceptor::CA_CERTIFICATE}"
        print_info
    ...
    def request_handler( req, res )
        ...
        sign_in_url = "#{p.scheme}://#{p.host}/scnr.engine.proxy.sign_in"
    ...
    #
    # @return   [Array<SCNR::Engine::Element::Form>]
    def find_login_form
    ...
    def update_forms( page, request, response )
        if (json = SCNR::Engine::Element::JSON.from_request( response.url, request ))
            page.jsons |= [json]
            return page
        end

        if (xml = SCNR::Engine::Element::XML.from_request( response.url, request ))
            page.xmls |= [xml]
            return page
        end
...
* [Control panel](http://scnr.engine.proxy/panel)
* [Shutdown URL](http://scnr.engine.proxy/shutdown)

_The above URLs will only work from a browser configured to use the proxy._

**SSL**

When browsing HTTPS sites, please accept the SCNR::Engine SSL certificate or install
...
`scnr.engine.proxy.session_token`, so if you choose to use a token to restrict
access to the proxy and need to pass traffic through the proxy programmatically
please configure your HTTP client with a cookie named `scnr.engine.proxy.session_token`
