...
class SCNR::Engine::Reporters::PDF < SCNR::Engine::Reporter::Base

    TEMPLATE_FILE = File.dirname( __FILE__ ) + '/pdf/default.erb'
    ...
    class TemplateScope
        ...
        def prep_description( str )
            escapeHTML SCNR::Engine::Reporters::HTML.prep_description( str )
        end
    ...
    def global_data
        ...
        SCNR::Engine::Issue::Severity::ORDER.each do |severity|
    ...
    def run
        ...
        print_line
        print_status 'Creating PDF report...'

        TemplateScope.global_data = global_data

        tmpdir = "#{SCNR::Engine::Options.paths.tmpdir}/#{generate_token}/"
    ...
    def self.info
        {
          name:         'PDF',
          description:  %q{Exports the audit results as a PDF report.},
          content_type: 'application/pdf',
          author:       'Tasos "Zapotek" Laskos <tasos.laskos@gmail.com>',
          version:      '0.1',
          options:      [ Options.outfile( '.pdf' ) ]
        }
    end
