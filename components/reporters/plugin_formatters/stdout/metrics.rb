...
class PluginFormatters::Metrics < SCNR::Engine::Plugin::Formatter
    ...
    def run
        ...
        print_info "Egress traffic:  #{SCNR::Engine::Utilities.bytes_to_megabytes general['egress_traffic']} MB"
        print_info "Ingress traffic: #{SCNR::Engine::Utilities.bytes_to_megabytes general['ingress_traffic']} MB"
        ...
        print_ok 'Scan'
        scan = results['scan']
        print_info "Duration:      #{SCNR::Engine::Utilities.seconds_to_hms scan['duration']}"
        ...
        print_info "Minimum response size: #{SCNR::Engine::Utilities.bytes_to_kilobytes http['response_size_min']} KB"
        print_info "Maximum response size: #{SCNR::Engine::Utilities.bytes_to_kilobytes http['response_size_max']} KB"
        print_info "Average response size: #{SCNR::Engine::Utilities.bytes_to_kilobytes http['response_size_average']} KB"
        print_info "Minimum request size:  #{SCNR::Engine::Utilities.bytes_to_kilobytes http['request_size_min']} KB"
        print_info "Maximum request size:  #{SCNR::Engine::Utilities.bytes_to_kilobytes http['request_size_max']} KB"
        print_info "Average request size:  #{SCNR::Engine::Utilities.bytes_to_kilobytes http['request_size_average']} KB"
        print_line
        ...
        results['platforms'].each do |type, platforms|
            ...
            platforms = platforms.map { |platform| SCNR::Engine::Platform::Manager::PLATFORM_NAMES[platform.to_sym] }
            print_info "#{SCNR::Engine::Platform::Manager::TYPES[type.to_sym]}: #{platforms.join( ', ' )}"
        end
    end
