...
class SCNR::Engine::Reporters::XML < SCNR::Engine::Reporter::Base

    LOCAL_SCHEMA  = File.dirname( __FILE__ ) + '/xml/schema.xsd'
    REMOTE_SCHEMA = 'https://raw.githubusercontent.com/placeholder/scnr/engine/' <<
        "v#{SCNR::Engine::VERSION}/components/reporters/xml/schema.xsd"
    NULL          = '[SCNR_ENGINE_NULL]'
    ...
    def run
        builder = Nokogiri::XML::Builder.new do |xml|
            xml.report(
                ...
                xml.status report.status
                xml.seed report.seed
                xml.options SCNR::Engine::Options.hash_to_save_data( report.options )
