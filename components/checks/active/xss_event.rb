...
class SCNR::Engine::Checks::XssEvent < SCNR::Engine::Check::Base
    ...
    def self.attribute_name
        'scnr_engine_xss_in_element_event'
    end
    ...
    def check_and_log( response, element )
        ...
        proof = SCNR::Engine::Parser.sax_parse(
            SAX.new( element.seed.split( ':', 2 ).last ),
            body
        ).proof

        return :checked if !proof

        log vector: element, response: response, proof: proof
    end
...
Engine has discovered that it is possible to insert script content directly into
an HTML event attribute. For example `<div onmouseover="x=INJECTION_HERE"</div>`,
where `INJECTION_HERE` represents the location where the Engine payload was detected.
},
                references:  {
