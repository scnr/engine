...
class SCNR::Engine::Checks::XssDomScriptContext < SCNR::Engine::Check::Base
    ...
    def self.options
        @options ||= {
            ...
            submit: {
                browser: {
                    parse_profile: Browser::ParseProfile.only(
                        :body, :execution_flow_sinks, :data_flow_sinks
                    )
                }
            }
        }
    ...
    def run
        ...
        each_candidate_dom_element do |element|
            ...
            options[:submit][:taint] = seed

            element.audit( taints, options )
        end
    end
...
SCNR::Engine has discovered that by modifying the affected DOM source, it is possible
