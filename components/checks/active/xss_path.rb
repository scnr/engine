...
class SCNR::Engine::Checks::XssPath < SCNR::Engine::Check::Base
    ...
    def check_and_log( response )
        ...
        return if !response.body.has_html_tag?( self.class.tag )

        return if !SCNR::Engine::Parser.sax_parse(
            SAX.new( self.class.tag ),
            response.body,
            whitelist: ['textarea', self.class.tag]
        ).found?
...
represents the location where the Engine payload was injected.
},
                references:  {
