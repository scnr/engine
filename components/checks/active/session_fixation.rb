...
class SCNR::Engine::Checks::SessionFixation < SCNR::Engine::Check::Base
    ...
    def run
        ...
        session.logged_in? do |logged_in|
            ...
            session.cookie do |cookie|
                ...
                audit( token, OPTIONS ) do |response, element|
...
SCNR::Engine has discovered that it is able to set its own session token.
},
                references:  {
