...
class SCNR::Engine::Checks::UnvalidatedRedirectDOM < SCNR::Engine::Check::Base
    ...
    def self.payloads
        @payloads ||= [
            ...
            "https://#{BASE_URL}",
            "http://#{BASE_URL}"
        ].map { |url| SCNR::Engine::URI( url ).to_s }
    end

    def self.payload?( url )
        (@set_payloads ||= Set.new( payloads )).include? SCNR::Engine::URI( url ).to_s
    end
    def payload?( url )
    ...
    def self.options
        @options ||= {
            ...
            submit: {
                browser:          {
                    parse_profile: Browser::ParseProfile.disable!
                }
            }
        }
    end
...
SCNR::Engine has discovered that the web page does not validate the parameter value prior
