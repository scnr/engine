...
class SCNR::Engine::Checks::MixedResource < SCNR::Engine::Check::Base
    ...
    def run
        ...
        page.document.nodes_by_name( 'script' ) do |script|
            url = script.attributes['src'].to_s
            log_resource( url ) if insecure_script?( script )
        end

        page.document.nodes_by_name( 'link' ) do |script|
    ...
    def insecure_script?( script )
        ...
        return if !parsed || parsed.domain == 'browser.scnr.engine'
...
Engine discovered that the affected site is utilising both HTTP and HTTPS. While
