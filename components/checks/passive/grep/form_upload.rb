...
class SCNR::Engine::Checks::FormUpload < SCNR::Engine::Check::Base
    ...
    def run
        page.forms.each do |form|
            form.inputs.keys.each do |name|
                ...
                form.node.nodes_by_attribute_name_and_value( 'type','file' ) do |node|
                    log(
                        proof: node.to_html,
                        vector: form
                    )
...
SCNR::Engine has flagged this not as a vulnerability, but as a prompt for the
