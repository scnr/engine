...
class SCNR::Engine::Checks::Captcha < SCNR::Engine::Check::Base
    ...
    def find_proof( node )
        node.nodes_by_name('input') do |input|
                html = input.to_html
...
Engine has flagged this not as a vulnerability, but as a prompt for the
penetration tester to conduct further manual testing on the CAPTCHA function, as
Engine cannot audit CAPTCHA protected forms.
