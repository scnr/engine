...
class SCNR::Engine::Checks::HttpPut < SCNR::Engine::Check::Base
    ...
    def self.body
        @body ||= 'Created by SCNR::Engine. ' + substring
    end

    def run
        path = "#{get_path( page.url )}SCNR_Engine-#{random_seed}"
...
Engine has discovered that the affected path allows clients to use the `PUT`
method. During this test, Engine has `PUT` a file on the server within the web
