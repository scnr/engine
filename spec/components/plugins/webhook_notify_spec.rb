    ...
    context 'when the content-type is JSON' do
        it 'sends a POST request at the end of the scan' do
            ...
            expect(actual_results['response']).to eq(
                ...
                "Content-Length: 174\r\n\r\n" <<
                "[\"application/json\",\"{\\\"SEED\\\":\\\"#{SCNR::Engine::Utilities.random_seed}\\\",\\\"URL\\\":\\\"#{options.url}\\\",\\\"MAX_SEVERITY\\\":\\\"\\\",\\\"ISSUE_COUNT\\\":\\\"0\\\",\\\"DURATION\\\":\\\"#{framework.report.delta_time}\\\"}\"]"
            )
        end
    end
    ...
    context 'when the content-type is XML' do
        it 'sends a POST request at the end of the scan' do
            ...
            expect(actual_results['response']).to eq(
                ...
                "X-Frame-Options: SAMEORIGIN\r\n" <<
                "Content-Length: 185\r\n\r\n" <<
                "[\"application/xml\",\"<SEED>#{SCNR::Engine::Utilities.random_seed}</SEED><URL>#{options.url}</URL><MAX_SEVERITY></MAX_SEVERITY><ISSUE_COUNT>0</ISSUE_COUNT><DURATION>#{framework.report.delta_time}</DURATION>\"]"
            )
        end
    end
end
