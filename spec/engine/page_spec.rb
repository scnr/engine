...
describe SCNR::Engine::Page do
    ...
    let(:url) { SCNR::Engine::Utilities.normalize_url web_server_url_for( :parser ) }
    ...
    it "supports #{SCNR::Engine::RPC::Serializer}" do
        page_with_nonces.forms = page_with_nonces.forms
        expect(page_with_nonces).to eq(SCNR::Engine::RPC::Serializer.deep_clone( page_with_nonces ))
    end
    ...
    describe '.from_rpc_data' do
        ...
        it 'restores Engine::Element::Form#node of #forms' do
            form = subject.forms.last
            expect(form.node).to be_kind_of SCNR::Engine::Parser::Nodes::Element
            expect(form.node).to be_truthy

            expect(restored.forms.last.node.to_s).to eq(form.node.to_s)
        end

        it 'restores Engine::Element::Link#node of #links' do
            link = subject.links.last
            expect(link.node).to be_kind_of SCNR::Engine::Parser::Nodes::Element
        ...
        describe 'option' do
            describe ':response' do
                it 'uses it to populate the page data' do
                    ...
                    parser = SCNR::Engine::Parser.new( response )
    ...
    describe '#performer' do
        it "returns the #{SCNR::Engine::HTTP::Request}#performer" do
    ...
    describe '#update_element_audit_whitelist' do
        context 'when passed a' do
            context 'Engine::Element::Capabilities::Auditable' do
    ...
    describe '#parser' do
        it 'is lazy-loaded' do
            ...
            expect(subject.parser).to be_kind_of SCNR::Engine::Parser
    ...
    [:links, :forms, :cookies, :headers].each do |element|
        ...
        describe "##{element}=" do
            element_klass = SCNR::Engine::Element.const_get( element.to_s[0...-1].capitalize )
    ...
    describe '#platforms' do
        it 'returns platforms for the given page' do
            expect(page.platforms).to be_kind_of SCNR::Engine::Platform::Manager
        end
    end
    ...
    describe '#text?' do
        context 'when the HTTP response is text/html' do
            it 'returns true' do
                expect(SCNR::Engine::Parser.new( Factory[:html_response] ).page.text?).to be_truthy
            end
        end

        context 'when the response is not text based' do
            it 'returns false' do
                expect(SCNR::Engine::Parser.new( Factory[:binary_response] ).page.text?).to be_falsey
            end
        end
    end
    ...
    describe '#==' do
        context 'when the pages are different' do
            it 'returns false' do
                ...
                p.links |= [SCNR::Engine::Element::Link.new( url: 'http://test.com', inputs: { 'test' => 'stuff' } )]
                p.forms |= [SCNR::Engine::Element::Form.new( url: 'http://test.com', inputs: { 'test' => 'stuff' } )]
                p.cookies |= [SCNR::Engine::Element::Cookie.new( url: 'http://test.com', inputs: { 'test' => 'stuff' } )]
                p.dom.push_transition described_class::DOM::Transition.new( "<a href='#' id='stuff'>", :onclick )

                c = p.dup
                c.links |= [SCNR::Engine::Element::Link.new( url: 'http://test.com', inputs: { 'test' => 'stuff2' } )]
                expect(c).not_to eq(p)

                c = p.dup
                c.forms |= [SCNR::Engine::Element::Form.new( url: 'http://test.com', inputs: { 'test' => 'stuff2' } )]
                expect(c).not_to eq(p)

                c = p.dup
                c.cookies |= [SCNR::Engine::Element::Cookie.new( url: 'http://test.com', inputs: { 'test' => 'stuff2' } )]
                expect(c).not_to eq(p)
        ...
        context 'when the pages are identical' do
            it 'returns true' do
                ...
                p.links |= [SCNR::Engine::Element::Link.new( url: 'http://test.com', inputs: { 'test' => 'stuff2' } )]
                p.forms |= [SCNR::Engine::Element::Form.new( url: 'http://test.com', inputs: { 'test' => 'stuff2' } )]
                p.cookies |= [SCNR::Engine::Element::Cookie.new( url: 'http://test.com', inputs: { 'test' => 'stuff2' } )]
    ...
    describe '#elements_within_scope' do
        it 'returns all elements that are within scope' do
            SCNR::Engine::Options.audit.elements :links, :forms, :cookies, :nested_cookies, :headers
...
EOHTML
    ...
    [:dup, :deep_clone].each do |method|
        describe "##{method}" do
            ...
            it 'preserves Engine::Element::Form#node of #forms' do
                form = subject.forms.last
                expect(form.node).to be_kind_of SCNR::Engine::Parser::Nodes::Element
                expect(form.node).to be_truthy
            ...
            it 'preserves Engine::Element::Link#node of #links' do
                link = subject.links.last
                expect(link.node).to be_kind_of SCNR::Engine::Parser::Nodes::Element
                expect(link.node).to be_truthy
    ...
    describe '.from_data' do
        it 'creates a page from the given data' do
            ...
            data = {
                ...
                links: [SCNR::Engine::Element::Link.new( elem_opts )],
                forms: [SCNR::Engine::Element::Form.new( elem_opts )],
                cookies: [SCNR::Engine::Element::Cookie.new( elem_opts )],
                cookie_jar: [
                    SCNR::Engine::Element::Cookie.new( elem_opts ),
                    SCNR::Engine::Element::Cookie.new( elem_opts )
                ],
                headers: [SCNR::Engine::Element::Header.new( elem_opts )],
                response: {
                    code: 200
                },
            ...
            page = SCNR::Engine::Page.from_data( data )
    ...
    describe '.from_response' do
        it 'creates a page from an HTTP response' do
            page = SCNR::Engine::Page.from_response( response )
            expect(page.class).to eq(SCNR::Engine::Page)
            parser = SCNR::Engine::Parser.new( response )
