...
describe SCNR::Engine::Parser do

    def normalize_url( *args )
        SCNR::Engine::Utilities.normalize_url( *args )
    end
    
    before :each do
        SCNR::Engine::Options.url = root_url
    ...
    let(:from_response) { described_class.new( response ) }
    let(:from_document) { described_class.new( document ) }
    let(:root_url) { normalize_url( web_server_url_for( :parser ) ) }
    let(:url) {  "#{root_url}?query_var_input=query_var_val" }
    let(:document) { described_class.parse response.body }
    let(:response) { SCNR::Engine::HTTP::Client.get( url, mode: :sync ) }

    describe '#initialize' do
        context 'Engine::Parser::Document' do
        ...
        context 'Array of Engine::HTTP::Response' do
            subject do
                described_class.new(
                    [response,
    ...
    describe '#link' do
        it 'returns the URL of the response as a Link' do
            expect(subject.link.action).to eq(root_url)
    ...
    describe '#body=' do
        let(:response) do
            SCNR::Engine::HTTP::Response.new(
                ...
                request: SCNR::Engine::HTTP::Request.new( url: root_url )
            )
        end
    ...
    describe '#page' do
        it 'returns a Page' do
            ...
            link = SCNR::Engine::Element::Link.new( url: url, inputs: subject.link_vars )
    ...
    describe '#cookies_to_be_audited' do
        ...
        it 'returns all system cookies for the domain' do
            SCNR::Engine::HTTP::Client.cookie_jar.update( SCNR::Engine::Element::Cookie.new(
                url:    'http://stuff/',
                inputs: {
                    irrelevant: 'iv',
                    cname:      'oldvar'
    ...
    describe '#text?' do
        context 'when the parser was initialized from an HTTP::Response' do
            ...
            context 'when the response is not text based' do
                let(:response) do
                    SCNR::Engine::HTTP::Response.new( url: url, headers: {
    ...
    describe '#links' do
        context 'when the response was a result of redirection' do
            let(:response) do
                url = 'http://stuff.com/'
                response = SCNR::Engine::HTTP::Response.new(
                    url: url,
                    body: '',
                    headers: {
                        'Content-Type' => 'text/html',
        ...
        context 'when the response is not text based' do
            let(:response) do
                SCNR::Engine::HTTP::Response.new( url: 'http://stuff', headers: {
                    'Content-Type' => 'bin/stuff'
                })
            end
    ...
    describe '#forms' do
        it 'returns an array of parsed forms' do
            ...
            expect(form.action).to eq(normalize_url( root_url + '/form' ))
            ...
            expect(form.action).to eq(normalize_url( root_url + '/form_2' ))
            expect(form.url).to eq(url)
        ...
        context 'when passed secondary responses' do
            it 'identifies the nonces' do
                ...
                responses << SCNR::Engine::HTTP::Client.get( root_url + 'with_nonce', mode: :sync )
                responses << SCNR::Engine::HTTP::Client.get( root_url + 'with_nonce', mode: :sync )

                parser = SCNR::Engine::Parser.new( responses )
                expect(parser.forms.map { |f| f.nonce_name }.sort).to eq(%w(nonce nonce2).sort)
            end
        end
        context 'when the response is not text based' do
            let(:response) do
                SCNR::Engine::HTTP::Response.new( url: url )
            end
    ...
    describe '#link_template' do
        context 'when the response url matches a link template' do
            ...
            before(:each) do
                SCNR::Engine::Options.audit.link_templates = /param\/(?<param>\w+)/
            end

            let(:response) do
                SCNR::Engine::HTTP::Response.new( url: root_url + 'test2/param/myvalue' )
            end

            it "returns a #{SCNR::Engine::Element::LinkTemplate}" do
    ...
    describe '#link_templates' do
        context 'when the response url matches a link template' do
            ...
            let(:response) do
                ...
                <html>
                    <body>
                        <a href="' + root_url + '/test2/param/myvalue"></a>
                    </body>
                </html>'
                )
            end
        ...
        context 'when it includes mailto: links' do
            let(:response) do
                ...
                <html>
                    <body>
                        <a href="' + root_url + '/test2/param/myvalue"></a>
                        <a href="mailto:nameaddress.com"></a>
                    </body>
                </html>'
                )
            end
            ...
            it 'ignores them' do
                expect(subject.paths).to eq([root_url+ 'test2/param/myvalue'])
            end
        end
        ...
        describe '#to_absolute' do
            it 'converts a relative path to absolute' do
                expect(subject.to_absolute( 'relative/path' )).to eq(
                    normalize_url( "#{root_url}/relative/path" )
                )
            end
        end
        ...
        describe '#paths' do
            it 'returns an array of all paths found in the page as absolute URLs' do
                paths = [
                    ...
                    "form",
                    "form_2",
                ].map { |p| normalize_url( root_url + '/' + p ) }
    ...
    context 'with base' do
        let(:url) do
            normalize_url( root_url + '/with_base?stuff=ha' )
        end
        let(:response) do
            SCNR::Engine::HTTP::Client.get( url, mode: :sync )
        end
        ...
        describe '#base' do
            it 'returns the base href attr' do
                expect(subject.base).to eq(normalize_url( "#{root_url}/this_is_the_base/" ))
            end
        end
        ...
        describe '#to_absolute' do
            it 'converts a relative path to absolute' do
                expect(subject.to_absolute( 'relative/path' )).to eq(
                    normalize_url( "#{subject.base}relative/path" )
                )
            end
        end
        ...
        describe '#links' do
            it 'returns an array of links' do
                ...
                link = links.first
                expect(link.action).to eq(root_url + 'with_base')
    ...
    describe '#headers' do
        it 'returns an array of headers' do
            subject.headers.each { |h| expect(h.class).to eq(SCNR::Engine::Element::Header) }
        end
        ...
        it "excludes #{SCNR::Engine::HTTP::Client::SEED_HEADER_NAME}" do
            subject.response.request.headers[SCNR::Engine::HTTP::Client::SEED_HEADER_NAME] = 'My-stuff'
            expect(subject.headers.find { |h| h.name == SCNR::Engine::HTTP::Client::SEED_HEADER_NAME }).to be_falsey
        end
    ...
    describe '#link_vars' do
        ...
        context "when there are #{SCNR::Engine::OptionGroups::Scope}#url_rewrites" do
            before :each do
                SCNR::Engine::Options.scope.url_rewrites = {
                    'stuff\/(\d+)' => '/stuff?id=\1'
                }
            end

            let(:response) do
                SCNR::Engine::HTTP::Client.get( "#{root_url}/stuff/13", mode: :sync )
            end
