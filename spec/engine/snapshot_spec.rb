...
describe SCNR::Engine::Snapshot do

    subject { described_class }
    let(:dump_archive) do
        "#{SCNR::Engine::Options.paths.tmpdir}/snapshot-#{SCNR::Engine::Utilities.generate_token}.#{described_class::EXTENSION}"
    end
    ...
    describe '.summary' do
        ...
        it 'includes :data' do
            expect(summary[:data]).to eq(SCNR::Engine::Data.statistics)
        end

        it 'includes :state' do
            expect(summary[:state]).to eq(SCNR::Engine::State.statistics)
        end
    end
    ...
    describe '.read_metadata' do
        ...
        it 'includes a :version' do
            expect(metadata[:version]).to eq(SCNR::Engine::VERSION)
        end
    ...
    describe '.dump' do
        it "stores #{SCNR::Engine::State} to disk" do
            expect(SCNR::Engine::State).to receive(:dump)
            expect(SCNR::Engine::Data).to receive(:dump)
    ...
    describe '.load' do
        it "stores #{SCNR::Engine::State} to disk" do
            ...
            expect(SCNR::Engine::State).to receive(:load)
            expect(SCNR::Engine::Data).to receive(:load)
