...
describe SCNR::Engine::OptionGroups::Paths do
    ...
    let(:paths_config_file) { "#{SCNR::Engine::Options.paths.tmpdir}/paths-#{Process.pid}.yml" }

    %w(root components logs checks reporters plugins services reports
    ...
    describe '#tmpdir' do
        context 'when no tmpdir has been specified via config' do
            it 'defaults to the OS tmpdir' do
                expect(subject.tmpdir).to eq SCNR::Engine.get_long_win32_filename( "#{Dir.tmpdir}/SCNR_Engine_#{Process.pid}" )
            end
        end
        ...
        context "when #{described_class}.config['tmpdir']" do
            it 'returns its value' do
                ...
                expect(subject.tmpdir).to eq("#{Dir.tmpdir}/my/tmpdir/SCNR_Engine_#{Process.pid}")
            end
        end
    end
    ...
    describe '.config' do
        ...
        it 'expands ~ to $HOME', if: !SCNR::Engine.windows? do
            yaml = {
