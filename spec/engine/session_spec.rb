...
describe SCNR::Engine::Session do

    subject { SCNR::Engine::Session.new }
    let(:url) { web_server_url_for( :session ) }
    let(:configured) do
        subject.configure(
            url:    "#{url}/login",
            inputs: {
    ...
    describe "#{SCNR::Engine::OptionGroups::Session}" do
    ...
    describe '#has_browser?' do
        context "when #{SCNR::Engine::OptionGroups::Scope}#dom_depth_limit is 0" do
            it 'returns false' do
    ...
    describe '#configuration' do
        it "returns #{SCNR::Engine::Data::Session}#configuration" do
            expect(subject.configuration.object_id).to eq(
                SCNR::Engine::Data.session.configuration.object_id
            )
        end
    end
        ...
        context 'when given a login sequence' do
            context 'when a browser is available' do
                ...
                it 'passes a browser instance' do
                    ...
                    expect(b).to be_kind_of SCNR::Engine::Browser
                end
                ...
                it 'updates the system cookies from the browser' do
                    ...
                    expect(SCNR::Engine::HTTP::Client.cookies.find { |c| c.name == 'foo' }).to be_truthy
                end
            end
        ...
        context 'when given login form info' do
            ...
            context 'when a browser is not available' do
                ...
                it 'uses the framework Page helpers' do
                    ...
                    expect(configured.login).to be_kind_of SCNR::Engine::Page
                    expect(configured).to be_logged_in
                end
            end
    ...
    describe '#cookie' do
        it 'returns the cookie that determines the login status' do
            ...
            SCNR::Engine::Options.session.check_url     = url + '/with_nonce'
            SCNR::Engine::Options.session.check_pattern = 'logged-in user'
    ...
    describe '#find_login_form' do
        ...
        context 'when passed a url' do
            it 'store the cookies set by that url' do
                ...
                expect(SCNR::Engine::HTTP::Client.cookies.find do |c|
                    c.name == 'you_need_to' && c.value == 'preserve this'
                end).to be_kind_of SCNR::Engine::Cookie
            end
