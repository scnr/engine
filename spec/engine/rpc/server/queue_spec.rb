...
require "#{SCNR::Engine::Options.paths.lib}/rpc/server/queue"

describe SCNR::Engine::RPC::Server::Queue do
    before( :each ) do
        SCNR::Engine::Options.system.max_slots    = 10
        SCNR::Engine::Options.queue.ping_interval = 0.5
    end
    ...
    context 'when there are queued scans' do
        it 'performs them' do
            ...
            subject.completed.values.each do |report|
                expect do
                    SCNR::Engine::Report.load( report )
                end.to_not raise_error
            end
        end

        it 'stores the reports' do
            subject.push( url: target )
            sleep 0.1 while subject.completed.empty?

            expect(subject.completed.values.first).to start_with SCNR::Engine::Options.paths.reports
        end
        ...
        it 'shuts down the Instance' do
            ...
            expect(SCNR::Engine::Processes::Manager.alive?( pid )).to be true

            sleep 0.1 while subject.completed.empty?
            sleep 2

            expect(SCNR::Engine::Processes::Manager.alive?( pid )).to be false
        end
    end

    context 'when a Dispatcher has been set' do
        subject { SCNR::Engine::Processes::Queues.spawn dispatcher: dispatcher.url }
        ...
        it 'sets OptionGroups::Queue#url' do
            id = subject.push( url: target )
            sleep 0.1 while subject.running.empty?

            info = subject.running[id]
            expect(instance_connect( info['url'], info['token'] ).queue_url).to eq subject.url
        end
    ...
    describe '#list' do
        it 'returns the queue entries grouped and sorted by priority' do
            medium = subject.push( url: target, priority: 0 )
            low    = subject.push( url: target, priority: -1 )
            high   = subject.push( url: target, priority: 1 )
    ...
    describe '#detach' do
        ...
        it 'removes OptionGroups::Queue#url' do
            id = subject.push( url: target )
            sleep 0.1 while subject.running.empty?
            ...
            expect(client.queue_url).to be_nil
        end
    ...
    describe '#attach' do
        let(:client) { instance_spawn }

        it 'attaches a running scan to the queue' do
        ...
        it 'sets OptionGroups::Queue#url' do
            ...
            expect(client.queue_url).to eq subject.url
        end

        context 'when the Instance is already attached to a Queue' do
            it 'returns false' do
                expect(client.queue_url).to be_nil
    ...
    describe '#get' do
        it "returns a queued scan's info" do
            info = subject.get( subject.push( url: target ) )
            expect(info).to eq(
                'options' => {
                    'url' => target
                },
    ...
    describe '#push' do
        ...
        context 'when priority is specified' do
            it 'uses it' do
                id = subject.push( url: target, priority: 1 )
                expect(subject.get( id )['priority']).to be 1
            end
        end

        context 'when given invalid scan options' do
            it 'raises an error' do
                expect do
                    subject.push( url: target, stuff: 1 )
