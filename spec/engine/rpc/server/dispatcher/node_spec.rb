...
describe SCNR::Engine::RPC::Server::Dispatcher::Node do

    def get_node( port = available_port )
        SCNR::Engine::Options.rpc.server_port = port

        SCNR::Engine::Processes::Manager.spawn( :node )

        c = SCNR::Engine::RPC::Client::Base.new(
            "#{SCNR::Engine::Options.rpc.server_address}:#{port}"
        )
