...
describe SCNR::Engine::System::Platforms::Windows, if: SCNR::Engine.windows? do
    it_should_behave_like 'SCNR::Engine::System::Platforms::Base'
    ...
    describe '.current?' do
        context 'when running on Windows' do
            it 'returns true'do
                expect(SCNR::Engine).to receive(:windows?).and_return( true )
        ...
        context 'when not running on Windows' do
            it 'returns false' do
                expect(SCNR::Engine).to receive(:windows?).and_return( false )
