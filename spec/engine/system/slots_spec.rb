...
describe SCNR::Engine::System::Slots do
    subject { system.slots }
    let(:system) { SCNR::Engine::System.instance }
    ...
    describe '#available' do
        context 'when OptionGroups::system#max_slots is set' do
            before do
                SCNR::Engine::Options.system.max_slots = 5
            end
        ...
        context 'when OptionGroups::system#max_slots is not set' do
            before do
                SCNR::Engine::Options.system.max_slots = nil
            end
    ...
    describe '#available_in_memory' do
        before do
            SCNR::Engine::Options.browser_cluster.pool_size = 6
        end
    ...
    describe '#memory_size' do
        ...
        it 'is approx 2.4GB with default options' do
            expect((memory_size / 1024.0 / 1024.0 / 1024.0).round(1)).to eq 2.4
        end

        context "when #{SCNR::Engine::OptionGroups::BrowserCluster}#pool_size is adjusted" do
            it 'adjusts the size' do
                prev = self.memory_size

                SCNR::Engine::Options.browser_cluster.pool_size = 3

                expect(subject.memory_size).to eq( prev - (2 * described_class::BROWSER_SIZE) )
            end
        end
    end
end
