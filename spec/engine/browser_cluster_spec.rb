...
describe SCNR::Engine::BrowserCluster do

    subject { described_class.new }
    let(:url) { SCNR::Engine::Utilities.normalize_url( web_server_url_for( :browser ) ) }
    let(:args) { [] }
    let(:job) do
        SCNR::Engine::BrowserCluster::Jobs::DOMExploration.new( job_options )
    end
    let(:job_options) do
        {
    ...
    describe '#initialize' do
        ...
        describe ':pool_size' do
            it 'sets the amount of browsers to instantiate' do
                expect(described_class.new( pool_size: 3 ).workers.size).to eq(3)
            end

            it "defaults to #{described_class::POOL_SIZE}#pool_size" do
