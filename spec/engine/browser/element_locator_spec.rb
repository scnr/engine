...
describe SCNR::Engine::Browser::ElementLocator do
    ...
    describe '#css' do
        ...
        context 'when there are multiple attributes' do
            ...
            context "and a #{described_class::ENGINE_ID}" do
                it "only includes the #{described_class::ENGINE_ID}" do
                    expect(described_class.new(
                       tag_name: :a,
                       attributes: {
                           stuff:  'blah',
                           stuff2: 'blah2',
                           described_class::ENGINE_ID =>     'my-id'
                       }
                   ).css).to eq("a[#{described_class::ENGINE_ID}=\"my-id\"]")
                end
            end
