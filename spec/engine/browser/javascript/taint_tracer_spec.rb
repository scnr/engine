...
describe SCNR::Engine::Browser::Javascript::TaintTracer do
    ...
    let(:url) { SCNR::Engine::Utilities.normalize_url( web_server_url_for( :taint_tracer ) ) }
    ...
    it 'is aliased to _token_taint_tracer' do
        load "debug?input=#{javascript.token}_taint_tracer.log_execution_flow_sink()"
        browser.watir.form.submit
        expect(subject.execution_flow_sinks).to be_any
    end

    it 'is aliased to _tokentainttracer' do
        load "debug?input=#{javascript.token}tainttracer.log_execution_flow_sink()"
        browser.watir.form.submit
        expect(subject.execution_flow_sinks).to be_any
    end
    ...
    describe '#taints=' do
        ...
        context 'when multiple taints are set' do
            it 'logs them in groups' do
                ...
                javascript.custom_code = subject.stub.function(
                    :taints=,
                    {
                        taint1 => {
                            trace: true
