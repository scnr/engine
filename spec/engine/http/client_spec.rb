...
describe SCNR::Engine::HTTP::Client do
    ...
    before( :each ) do
        ...
        SCNR::Engine::Options.audit.links = true
    end

    subject { described_class }
    let(:url) { SCNR::Engine::URI.normalize web_server_url_for( :client ) }
    ...
    it 'preserves set-cookies' do
        body = nil
        subject.get( url + 'set_and_preserve_cookies', update_cookies: true )
        subject.run
    ...
    describe '#reset_options' do
        it 'resets #max_concurrency' do
            SCNR::Engine::Options.http.request_concurrency = 10
        ...
        it 'resets User-Agent headers' do
            SCNR::Engine::Options.device.user_agent = 'Stuff'
        ...
        it 'resets custom headers' do
            SCNR::Engine::Options.http.request_headers = {
                'X-Stuff' => '1'
            }
    ...
    describe 'SCNR::Engine::Options' do
        describe '#url' do
            context 'when the target URL includes auth credentials' do
                it 'uses them globally' do
                    ...
                    code = 0
                    subject.get( "#{url}auth/simple-chars" ) { |res| code = res.code }
                    subject.run
                    expect(code).to eq(401)

                    u = SCNR::Engine::Utilities.uri_parse( "#{url}auth/simple-chars" )
        ...
        describe '#fingerprint?' do
            context 'true' do
                before do
                    SCNR::Engine::Options.paths.fingerprinters = nil
                end

                it 'performs platform fingerprinting on the response' do
                    SCNR::Engine::Options.fingerprint
            ...
            context 'false' do
                it 'does not fingerprint the response' do
                    SCNR::Engine::Options.do_not_fingerprint
    ...
    describe 'SCNR::Engine::OptionGroups::HTTP' do
        describe '#request_concurrency' do
            context 'Integer' do
                it 'uses it as a max_concurrency' do
                    SCNR::Engine::Options.http.request_concurrency = 34
                    subject.reset
        ...
        describe '#request_queue_size' do
            context 'when reached' do
                it 'performs an emergency run' do
                    SCNR::Engine::Options.http.request_queue_size = 10
        ...
        describe '#request_timeout' do
            context 'Integer' do
                it 'uses it as an HTTP timeout' do
                    SCNR::Engine::Options.http.request_timeout = 10000000000
                    timed_out = false
                    subject.request( url + '/sleep' ) { |res| timed_out = res.timed_out? }
                    subject.run
                    expect(timed_out).to be_falsey

                    SCNR::Engine::Options.http.request_timeout = 1
                    subject.reset
                    timed_out = false
                    subject.request( url + '/sleep' ) { |res| timed_out = res.timed_out? }
                    subject.run
        ...
        describe '#authentication_username and #authentication_password' do
            it 'uses them globally' do
                ...
                # first fail to make sure that our test server is actually working properly
                code = 0
                subject.get( url + 'auth/weird-chars' ) { |res| code = res.code }
                subject.run
                expect(code).to eq(401)
        ...
        describe 'user_agent' do
            it 'uses the default user-agent setting' do
                ...
                expect(SCNR::Engine::Options.device.user_agent).to eq(SCNR::Engine::OptionGroups::Device.defaults[:user_agent])
            end
            context 'String' do
                it 'uses it as a user-agent' do
                    ua = 'my user agent'
                    SCNR::Engine::Options.device.user_agent = ua.dup
                    subject.reset
        ...
        describe '#request_redirect_limit' do
            context 'Integer' do
                it 'should not exceed that amount of redirects' do
                    SCNR::Engine::Options.http.request_redirect_limit = 2
                    code = nil
                    subject.get( url + 'redirect', follow_location: true ) { |res| code = res.code }
                    subject.run
                    expect(code).to eq(302)

                    SCNR::Engine::Options.http.request_redirect_limit = 10
                    subject.reset
    ...
    describe '#headers' do
        it 'provides access to default headers' do
            ...
            expect(headers['User-Agent']).to eq('Mozilla/5.0 (Gecko) SCNR::Engine/v' + SCNR::Engine::VERSION)
        end
        ...
        context "when #{SCNR::Engine::OptionGroups::HTTP}#request_headers is set" do
            it 'includes them' do
                ...
                subject.reset
                headers = subject.headers
                expect(headers['From']).to eq(SCNR::Engine::Options.http.request_headers['From'])
                expect(headers['Accept']).to eq('text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8')
                expect(headers['User-Agent']).to eq(SCNR::Engine::Options.http.request_headers['User-Agent'])
            end
        end

        context 'when the authorized_by option is set' do
            it 'includes it in the From field' do
                SCNR::Engine::Options.authorized_by = 'The Dude'
                subject.reset
                expect(subject.headers['From']).to eq(SCNR::Engine::Options.authorized_by)
            end
        end
    end
    ...
    describe '#cookie_jar' do
        ...
        context "when #{SCNR::Engine::OptionGroups::HTTP}#cookie_jar_filepath is set" do
            it 'adds the contained cookies to the CookieJar' do
                SCNR::Engine::Options.http.cookie_jar_filepath = fixtures_path + 'cookies.txt'
                expect(subject.cookie_jar.cookies).to be_empty
                subject.reset

                cookies = subject.cookie_jar.cookies
                expect(cookies.size).to eq(4)
                expect(cookies).to eq(SCNR::Engine::Utilities.cookies_from_file( '', SCNR::Engine::Options.http.cookie_jar_filepath ))
            end

            context 'but the path is invalid' do
                it 'raises CNR::Engine::HTTP::CookieJar::Error::CookieJarFileNotFound' do
                    SCNR::Engine::Options.http.cookie_jar_filepath = fixtures_path + 'cookies.does_not_exist.txt'
                    expect{ subject.reset }.to raise_error SCNR::Engine::HTTP::CookieJar::Error::CookieJarFileNotFound
                    ap 1111
        ...
        context 'when the cookies option is set' do
            it 'adds those cookies to the CookieJar' do
                SCNR::Engine::Options.http.cookies = {
                    'cookie1' => 'val1',
                    'cookie2' => 'val2',
                }
        ...
        context "when #{SCNR::Engine::OptionGroups::HTTP}#cookie_string is set" do
            it 'parses the string and add those cookies to the CookieJar' do
                SCNR::Engine::Options.http.cookie_string = 'my_cookie_name=val1;path=/my/path,blah_name=val2, stuff=%25blah, another_name=another_val'
                expect(subject.cookie_jar.cookies).to be_empty
                subject.reset
                cookies = subject.cookie_jar.cookies
                expect(cookies.size).to eq(4)
    ...
    describe '#cookies' do
        it 'returns the current cookies' do
            SCNR::Engine::Options.http.cookie_string = 'my_cookie_name=val1,blah_name=val2, another_name=another_val'
            expect(subject.cookie_jar.cookies).to be_empty
            subject.reset
    ...
    describe '#request' do
        it "uses the URL in #{SCNR::Engine::Options}#url as a default" do
            ...
            subject.run
            expect(url.start_with?( url.to_s )).to be_truthy
        end
        ...
        describe ':response_max_size' do
            context 'when not specified' do
                context "and #{SCNR::Engine::OptionGroups::HTTP}#response_max_size is specified" do
                    context 'when response bodies are larger that its value' do
                        it 'ignores them' do
                            SCNR::Engine::Options.http.response_max_size = 0
                            expect(subject.request( url + '/http_response_max_size',
                                             mode: :sync
                            ).body).to be_empty

                            SCNR::Engine::Options.http.response_max_size = 1
                            expect(subject.request( url + '/http_response_max_size',
                                             mode: :sync
                            ).body).to be_empty

                            SCNR::Engine::Options.http.response_max_size = 999999
                            expect(subject.request( url + '/http_response_max_size',
                                             mode: :sync
                            ).body).to be_empty
                        end
                    ...
                    context 'when response bodies are not larger that its value' do
                        it 'reads them' do
                            SCNR::Engine::Options.http.response_max_size = 1000000
                            expect(subject.request( url + '/http_response_max_size',
                                             mode: :sync
        ...
        describe ':timeout' do
            ...
            describe 'Numeric' do
                it 'sets a timeout value in milliseconds' do
                    timed_out = false
                    subject.request( url + '/sleep', timeout: 3_000 ) { |res| timed_out = res.timed_out? }
                    subject.run
                    expect(timed_out).to be_truthy
            ...
            describe 'true' do
                it 'updates the cookie_jar' do
                    ...
                    cookies << SCNR::Engine::Element::Cookie.new(
                        ...
                        name:   'key2',
                        value:  'val2',
                        domain: SCNR::Engine::URI( url ).domain
                    )
    ...
    describe '#parse_and_set_cookies' do
        it 'updates the cookies from a response and call on_new_cookies blocks' do
            ...
            expect(SCNR::Engine::Options.http.cookies).to be_empty
            expect(subject.cookies).to be_empty
            subject.parse_and_set_cookies( res )
            expect(subject.cookies).to eq(cookies)
        end
