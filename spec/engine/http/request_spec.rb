...
describe SCNR::Engine::HTTP::Request do
    it_should_behave_like 'SCNR::Engine::HTTP::Message'
    ...
    describe '#initialize' do
        it 'sets the instance attributes by the options' do
            r = described_class.new( options )
            expect(r.url).to          eq(SCNR::Engine::Utilities.normalize_url( url ))
    ...
    describe '#id' do
        it 'is incremented by the Engine::HTTP::Client' do
            10.times do |i|
                expect(client.get( url ).id).to eq(i)
            end
        end
    end
    ...
    describe '#to_typhoeus' do
        ...
        context "when configured with a #{SCNR::Engine::OptionGroups::HTTP}#proxy_host/#{SCNR::Engine::OptionGroups::HTTP}#proxy_port" do
            before :each do
                SCNR::Engine::Options.http.proxy_host = 'stuff'
                SCNR::Engine::Options.http.proxy_port = '8080'
            end
            ...
            context "and #{SCNR::Engine::OptionGroups::HTTP}#proxy_username/#{SCNR::Engine::OptionGroups::HTTP}#proxy_password" do
                it 'forwards it' do
                    SCNR::Engine::Options.http.proxy_username = 'name'
                    SCNR::Engine::Options.http.proxy_password = 'secret'

                    expect(subject.options[:proxyuserpwd]).to eq('name:secret')
                end
            end

            context "and #{SCNR::Engine::OptionGroups::HTTP}#proxy_type" do
                it 'forwards it' do
                    SCNR::Engine::Options.http.proxy_type = 'http'
                    expect(subject.options[:proxytype]).to eq(:http)
                end
            end
        end
        ...
        context "and #{SCNR::Engine::OptionGroups::HTTP}#authentication_username/#{SCNR::Engine::OptionGroups::HTTP}#authentication_password" do
            before :each do
                SCNR::Engine::Options.http.authentication_username = 'name'
                SCNR::Engine::Options.http.authentication_password = 'secret'
            end
        ...
        context "#{SCNR::Engine::OptionGroups::HTTP}#authentication_type" do
            before :each do
                SCNR::Engine::Options.http.authentication_username = 'name'
                SCNR::Engine::Options.http.authentication_password = 'secret'
                SCNR::Engine::Options.http.authentication_type     = 'ntlm'
            end
            ...
            it 'defaults to :auto' do
                SCNR::Engine::Options.http.authentication_type = nil

                expect(subject.options[:httpauth]).to eq(:auto)
            end
        end

        context "#{SCNR::Engine::OptionGroups::HTTP}#response_max_size" do
            before :each do
                SCNR::Engine::Options.http.response_max_size = 10
            end
        ...
        context "#{SCNR::Engine::OptionGroups::HTTP}#ssl_verify_peer" do
            context 'true' do
                it "sets #{Typhoeus::Request}#options[:ssl_verifypeer]" do
                    SCNR::Engine::Options.http.ssl_verify_peer = true
            ...
            context 'false' do
                it "sets #{Typhoeus::Request}#options[:ssl_verifypeer]" do
                    SCNR::Engine::Options.http.ssl_verify_peer = false
                    expect(subject.options[:ssl_verifypeer]).to eq(false)
                end
            end
        end

        context "#{SCNR::Engine::OptionGroups::HTTP}#ssl_verify_host" do
            context 'true' do
                it "sets #{Typhoeus::Request}#options[:ssl_verifyhost] to 2" do
                    allow(SCNR::Engine::Options.http).to receive(:ssl_verify_host){ true }
            ...
            context 'false' do
                it "sets #{Typhoeus::Request}#options[:ssl_verifyhost] to 2" do
                    allow(SCNR::Engine::Options.http).to receive(:ssl_verify_host){ false }
                    expect(subject.options[:ssl_verifyhost]).to eq(0)
                end
            end
        end

        context "#{SCNR::Engine::OptionGroups::HTTP}#ssl_certificate_filepath" do
            it "sets #{Typhoeus::Request}#options[:sslcert]" do
                allow(SCNR::Engine::Options.http).to receive(:ssl_certificate_filepath){ :stuff }
                expect(subject.options[:sslcert]).to eq(:stuff)
            end
        end

        context "#{SCNR::Engine::OptionGroups::HTTP}#ssl_certificate_type" do
            it "sets #{Typhoeus::Request}#options[:sslcerttype]" do
                allow(SCNR::Engine::Options.http).to receive(:ssl_certificate_type){ :stuff }
                expect(subject.options[:sslcerttype]).to eq(:stuff)
            end
        end

        context "#{SCNR::Engine::OptionGroups::HTTP}#ssl_key_filepath" do
            it "sets #{Typhoeus::Request}#options[:sslkey]" do
                allow(SCNR::Engine::Options.http).to receive(:ssl_key_filepath){ :stuff }
                expect(subject.options[:sslkey]).to eq(:stuff)
            end
        end

        context "#{SCNR::Engine::OptionGroups::HTTP}#ssl_key_type" do
            it "sets #{Typhoeus::Request}#options[:sslkeytype]" do
                allow(SCNR::Engine::Options.http).to receive(:ssl_key_type){ :stuff }
                expect(subject.options[:sslkeytype]).to eq(:stuff)
            end
        end

        context "#{SCNR::Engine::OptionGroups::HTTP}#ssl_key_password" do
            it "sets #{Typhoeus::Request}#options[:sslkeypasswd]" do
                allow(SCNR::Engine::Options.http).to receive(:ssl_key_password){ :stuff }
                expect(subject.options[:sslkeypasswd]).to eq(:stuff)
            end
        end

        context "#{SCNR::Engine::OptionGroups::HTTP}#ssl_ca_filepath" do
            it "sets #{Typhoeus::Request}#options[:cainfo]" do
                allow(SCNR::Engine::Options.http).to receive(:ssl_ca_filepath){ :stuff }
                expect(subject.options[:cainfo]).to eq(:stuff)
            end
        end

        context "#{SCNR::Engine::OptionGroups::HTTP}#ssl_ca_directory" do
            it "sets #{Typhoeus::Request}#options[:capath]" do
                allow(SCNR::Engine::Options.http).to receive(:ssl_ca_directory){ :stuff }
                expect(subject.options[:capath]).to eq(:stuff)
            end
        end

        context "#{SCNR::Engine::OptionGroups::HTTP}#ssl_version" do
            it "sets #{Typhoeus::Request}#options[:sslversion]" do
                allow(SCNR::Engine::Options.http).to receive(:ssl_version){ :stuff }
