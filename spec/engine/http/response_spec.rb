...
describe SCNR::Engine::HTTP::Response do
    ...
    let(:url) { web_server_url_for( :client ) }
    let(:client) { SCNR::Engine::HTTP::Client }
    subject { client.get( url, mode: :sync ) }
    ...
    describe '#status_line' do
        it 'returns the first line of the response' do
            expect(client.get( url, mode: :sync ).status_line).to eq('HTTP/1.1 200 OK')
        end
    end
    ...
    describe '#platforms' do
        it 'returns the platform manager for the resource' do
            expect(Factory[:response].platforms).to be_kind_of SCNR::Engine::Platform::Manager
        end
    end
    ...
    describe '#app_time' do
        it 'returns the approximated webap pprocessing time' do
            ...
            response = client.get( "#{url}/sleep", mode: :sync )
            expect(response.app_time).to be > 4
            expect(response.app_time).to be < 4.01
        end
    end
    ...
    describe '#partial?' do
        context 'when the response body does not match the content-length' do
            it 'returns true' do
                response = client.get( "#{url}/partial", mode: :sync )
        ...
        context 'when dealing with a stream' do
            context 'that does not complete' do
                it 'returns true' do
                    response = client.get( "#{url}/partial_stream", mode: :sync )
            ...
            context 'that closes abruptly' do
                it 'returns true' do
                    response = client.get( "#{url}/fail_stream", mode: :sync )
            ...
            context 'that completes' do
                it 'returns false' do
                    response = client.get( "#{url}/stream", mode: :sync )
                    expect(response).to_not be_partial
                end
            end
        end
    ...
    describe '#to_page' do
        it 'returns an SCNR::Engine::Page based on the response data' do
            body = <<-EOHTML
