...
describe SCNR::Engine::HTTP::Response::Scope do
    ...
    describe '#out?' do
        ...
        context "when #{SCNR::Engine::URI::Scope}#out?" do
            context 'true' do
                it 'returns true' do
                    # We can't stub #out? because we also override it.
                    allow_any_instance_of(SCNR::Engine::URI::Scope).to receive(:exclude?) { true }
            ...
            context 'false' do
                it 'returns false' do
                    allow_any_instance_of(SCNR::Engine::URI::Scope).to receive(:exclude?) { false }
                    expect(subject.out?).to be_falsey
                end
            end
        end
    ...
    describe '#exclude_as_binary?' do
        context 'when #text?' do
            context 'true' do
                context "and #{SCNR::Engine::OptionGroups::Scope}#exclude_binaries?" do
                    context 'true' do
                        it 'returns false' do
            ...
            context 'false' do
                context "and #{SCNR::Engine::OptionGroups::Audit}#exclude_binaries?" do
                    context 'true' do
                        it 'returns true' do
                            scope.exclude_binaries = true
    ...
    describe '#exclude_content?' do
        context "when #{SCNR::Engine::OptionGroups::Scope}#exclude_content_patterns" do
