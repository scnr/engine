...
describe SCNR::Engine::HTTP::ProxyServer do

    let(:root_url) { web_server_url_for( :proxy_server ) + '/' }
    let(:url) { root_url }
    let(:address) { Socket.ip_address_list.find(&:ipv4?).ip_address }
    let(:port) { SCNR::Engine::Utilities.available_port }
    ...
    describe '#initialize' do
        ...
        describe ':port' do
            it 'sets the listen port' do
                port = SCNR::Engine::Utilities.available_port
        ...
        describe ':timeout' do
            it 'sets the HTTP request timeout' do
                ...
                expect(Typhoeus::Request.get( sleep_url ).code).to eq(0)
        ...
        describe ':request_handler' do
            it 'sets a block to handle each HTTP request before the request is forwarded to the origin server' do
                ...
                proxy = described_class.new(
                    request_handler: proc do |request, _|
                        expect(request).to be_kind_of SCNR::Engine::HTTP::Request
                        called = true
                    end
                )
                proxy.start_async
            ...
            it 'assigns the request to the response' do
                ...
                proxy = described_class.new(
                    request_handler: proc do |_, response|
                        expect(response.request).to be_kind_of SCNR::Engine::HTTP::Request
                        called = true
                    end
                )
                proxy.start_async
