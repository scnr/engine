...
describe SCNR::Engine::Element::Cookie do
    ...
    subject do
        ...
        ).tap { |e| e.skip_dom = false }
    end

    it 'should be assigned to Engine::Cookie for easy access' do
        expect(SCNR::Engine::Cookie).to eq(described_class)
    end
        ...
        context 'domain' do
            ...
            context 'missing' do
                ...
                it 'sets it to the URL host' do
                    expect(subject.domain).to eq SCNR::Engine::URI(url).host
                end
            end
        end
    end
    ...
    describe '#mutations' do
        ...
        describe 'Options.audit.cookies_extensively' do
            let(:page) { SCNR::Engine::Page.from_url( url + 'with_other_elements' ) }
            subject { page.cookies.first.tap { |c| c.auditor = auditor } }

            it 'submits the default elements of the page along with the cookie mutations' do
                expect(subject.mutations( 'seed' ).map { |e| e.type }.uniq.size).to eq(1)

                mutations = subject.mutations( 'seed' ).map { |e| e.inputs }

                SCNR::Engine::Options.audit.cookies_extensively = true

                expect(subject.mutations( 'seed' ).map { |e| e.type }.uniq.size).to be > 1
                subject.mutations( 'seed' ).each do |e|
                    next if e.is_a? described_class

                    expect(mutations).to include e.audit_options[:submit][:cookies]
                end

                SCNR::Engine::Options.audit.cookies_extensively = false
                expect(subject.mutations( 'seed' ).map { |e| e.type }.uniq.size).to eq(1)
            end
        end
    end
    ...
    describe '#data' do
        it 'returns the cookie data' do
            expect(subject.data).to eq({
                ...
                secure:      nil,
                path:        '/submit',
                domain:      SCNR::Engine::URI(url).host,
                httponly:    false
            })
        end
    end
    ...
    describe '#skip_dom?' do
        ...
        context 'by default' do
            it 'returns true' do
                expect(subject).to be_skip_dom
            end
        end

        context 'when false' do
            before { subject.skip_dom = false }

            it 'returns false' do
                expect(subject).not_to be_skip_dom
            end
        end

        context 'when true' do
            before { subject.skip_dom = true }
    ...
    describe '.decode' do
        it 'delegates to Form.decode' do
            ...
            allow(SCNR::Engine::Form).to receive(:decode) { 'ret' }.with( string )
            expect(described_class.decode( string )).to eq('ret')
        end
    end

    describe '#to_set_cookie' do
        it 'removes trailing slashes in Path' do
        ...
        context 'when the cookie is for all subdomains' do
            it 'includes a Domain attribute' do
                ...
                expect(described_class.from_set_cookie( url, c.to_set_cookie ).first).to eq(c)
                expect(c.to_set_cookie).to eq(
                    'blah%3Dha%25=some+stuff+%3B; Path=/; Domain=.localhost'
                )
            end
        end
    ...
    describe '.from_file' do
        it 'parses a Netscape cookie_jar file into an array of cookies' do
            ...
            expect(cookie.secure).to eq(false)
            expect(cookie.session?).to eq(false)
            expect(cookie.expires).to eq(Time.parse( '2052-04-17 17:46:00 +0200' ))
