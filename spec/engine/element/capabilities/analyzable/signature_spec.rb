...
describe SCNR::Engine::Element::Capabilities::Analyzable::Signature do
    ...
    let(:positive){ SCNR::Engine::Element::Link.new( url: url, inputs: { 'input' => '' } ).tap { |e| e.auditor = auditor } }
    ...
    describe '#signature_analysis' do
        ...
        context 'when the element action matches a skip rule' do
            it 'returns false' do
                auditable = SCNR::Engine::Element::Link.new(
                    url: 'http://stuff.com/',
                    inputs: { 'input' => '' }
                )
