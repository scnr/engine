...
describe SCNR::Engine::Element::XML do
    inputtable_source = '<input1>value1</input1>'
    with_sinks_source = '<xml><active>value1</active><blind>value2</blind></xml>'

    it_should_behave_like 'element'

    it_should_behave_like 'with_source'
    it_should_behave_like 'with_auditor'
    it_should_behave_like 'with_sinks', inputs: described_class.parse_inputs( with_sinks_source )
    ...
    let(:inputtable) do
        described_class.new(
            url:      "#{url}submit",
            source: inputtable_source
        )
    end
    let(:with_sinks) do
        described_class.new(
            url:    url,
            source: with_sinks_source
        )
    end
...
EOXML
    ...
    it 'is assigned to SCNR::Engine::Link for easy access' do
        expect(SCNR::Engine::XML).to eq(described_class)
    end
        ...
        describe ':source' do
            ...
            context 'when missing' do
                it 'fails' do
                    ...
                    end.to raise_error SCNR::Engine::Element::Capabilities::WithSource::Error::MissingSource
                end
            end
        end
    end
