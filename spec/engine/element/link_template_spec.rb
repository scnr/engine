...
describe SCNR::Engine::Element::LinkTemplate do
    ...
    it_should_behave_like 'with_sinks', base_url: "active/value1/blind/value2"
    ...
    let(:with_sinks) do
        described_class.new(
            url:      "#{url}active/value1/blind/value2",
            template: /active\/(?<active>\w+)\/blind\/(?<blind>\w+)/
        )
    end
        ...
        describe ':options' do
            ...
            context 'when no :inputs are provided' do
                ...
                context 'when no :template is provided' do
                    it "tries to match one of #{SCNR::Engine::OptionGroups::Audit}#link_templates" do
                        SCNR::Engine::Options.audit.link_templates = template
    ...
    describe '#coverage_id' do
        it "takes into account #{described_class::DOM}#template names" do
            ...
            SCNR::Engine::Options.audit.link_templates << /param2\/(?<param2>\w+)/
    ...
    describe '.extract_inputs' do
        ...
        context 'when no templates are given' do
            it "defaults to #{SCNR::Engine::OptionGroups::Audit}#link_templates" do
                url       = "#{url}input1/value1/input2/value2"
                templates = [/input1\/(?<input1>\w+)\/input2\/(?<input2>\w+)/]

                SCNR::Engine::Options.audit.link_templates = templates
                ...
                expect([templates]).to eq([SCNR::Engine::Options.audit.link_templates])
            end
        end
    ...
    describe '.from_parser' do
        ...
        context 'when links have actions that are out of scope' do
            ...
            it 'ignores them' do
                SCNR::Engine::Options.scope.exclude_path_patterns = [/exclude/]
