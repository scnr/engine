...
describe SCNR::Engine::Element::Server do
    ...
    describe '#log_remote_file_if_exists' do
        let(:base_url) { url + '/log_remote_file_if_exists/' }
        ...
        context 'when a remote file exists' do
            it 'logs an issue' do
                ...
                expect(logged_issue.vector.class).to eq(SCNR::Engine::Element::Server)
            ...
            it 'assigns the extra Issue options' do
                ...
                framework.http.run
                expect(SCNR::Engine::Data.issues.sort.first).to_not be_trusted
            end

            context 'when one issue is logged' do
                it "does not push the response to the #{SCNR::Engine::Trainer}" do
            ...
            context 'when multiple issues are logged' do
                it "pushes the responses to the #{SCNR::Engine::Trainer}" do
                    auditable.log_remote_file_if_exists( base_url + 'true' )
        ...
        context 'when a remote file does not exist' do
            ...
            it "does not push the responses to the #{SCNR::Engine::Trainer}" do
                auditable.log_remote_file_if_exists( base_url + 'false' )
    ...
    describe '#remote_file_exist?' do
        ...
        context 'without a custom 404 handler' do
            it 'does not perform fingerprinting' do
                ...
                expect(request.response.code).to be 404
        ...
        context 'with a custom 404 handler' do
            let(:_404_url) { base_url + 'custom_404/' }
