...
describe SCNR::Engine::URI::Scope do
    ...
    describe '#too_deep?' do
        subject { SCNR::Engine::URI.parse( '/very/very/very/very/deep' ).scope }

        context 'when the directory depth of the URL is' do
            context "less than #{SCNR::Engine::OptionGroups::Scope}#directory_depth_limit" do
                it 'returns false' do
        ...
        context "when #{SCNR::Engine::OptionGroups::Scope}#directory_depth_limit has not been configured" do
            it 'returns false' do
                expect(subject.too_deep?).to be_falsey
            end
        end
    ...
    describe '#redundant?' do
        subject { SCNR::Engine::URI.parse( 'http://stuff.com/match_this' ).scope }
    ...
    describe '#auto_redundant?' do
        subject { SCNR::Engine::URI( 'http://test.com/?test=2&test2=2').scope }
        ...
        describe 'when the URL has no parameters' do
            subject { SCNR::Engine::URI( 'http://test.com/').scope }
    ...
    describe '#exclude_file_extension?' do
        subject { SCNR::Engine::URI.parse( 'http://test.com/exclude.gif' ).scope }
    ...
    describe '#in_domain?' do
        ...
        let(:with_subdomain) { SCNR::Engine::URI.parse( url_with_subdomain ).scope }
        let(:with_same_subdomain) { SCNR::Engine::URI.parse( url_with_same_subdomain ).scope }
        let(:with_different_subdomain) { SCNR::Engine::URI.parse( url_with_different_subdomain ).scope }
        let(:without_subdomain) { SCNR::Engine::URI.parse( url_without_subdomain ).scope }

        context "when #{SCNR::Engine::OptionGroups::Scope}#include_subdomains is" do
            context 'true' do
                before :each do
                    scope.include_subdomains = true
                end

                context "when #{SCNR::Engine::Options}#url" do
                    context 'has a subdomain' do
                        before :each do
                            SCNR::Engine::Options.url = url_with_subdomain
                        end
                    ...
                    context 'has no a subdomain' do
                        before :each do
                            SCNR::Engine::Options.url = url_without_subdomain
                        end
    ...
    describe '#follow_protocol?' do
        let(:http) { SCNR::Engine::URI.parse( 'http://test2.com/blah/ha' ).scope }
        let(:https) { SCNR::Engine::URI.parse( 'https://test2.com/blah/ha' ).scope }
        let(:other) { SCNR::Engine::URI.parse( 'stuff://test2.com/blah/ha' ).scope }

        context 'when the reference URL uses' do
            context 'HTTPS' do
                before :each do
                    SCNR::Engine::Options.url = 'https://test2.com/blah/ha'
                end
            ...
            context 'HTTP' do
                before :each do
                    SCNR::Engine::Options.url = 'http://test2.com/blah/ha'
                end
    ...
    describe '#in?' do
        subject { SCNR::Engine::URI.parse( 'http://stuff/' ).scope }
