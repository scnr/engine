...
describe SCNR::Engine::Page::DOM do

    def create_page( options = {} )
        SCNR::Engine::Page.new response: SCNR::Engine::HTTP::Response.new(
    ...
    let(:root_url) { SCNR::Engine::Utilities.normalize_url( web_server_url_for( :page_dom ) ) }
    ...
    it 'Marshaling ignores the page' do
        expect(subject.page).to be_kind_of SCNR::Engine::Page
    ...
    describe '#restore' do
        context 'when the state can be restored by #url' do
            it 'loads the #url' do
                url = "#{root_url}restore/by-url"

                browser.load "#{root_url}restore/by-url"
        ...
        context 'when the state cannot be restored by URL' do
            it 'replays its #transitions' do
                url = "#{root_url}restore/by-transitions"
        ...
        context 'when a transition could not be replayed' do
            it 'returns nil' do
                allow_any_instance_of(SCNR::Engine::Page::DOM::Transition).to receive(:play){ false }

                browser.load "#{root_url}restore/by-transitions"
                page = browser.explore_and_flush.last
