...
describe SCNR::Engine::Page::Scope do
    ...
    describe '#out?' do
        ...
        context "when #{SCNR::Engine::HTTP::Response::Scope}#out?" do
            context 'true' do
                it 'returns true' do
                    # We can't stub #out? because we also override it.
                    allow_any_instance_of(SCNR::Engine::HTTP::Response::Scope).to receive(:exclude?) { true }
            ...
            context 'false' do
                it 'returns false' do
                    allow_any_instance_of(SCNR::Engine::HTTP::Response::Scope).to receive(:exclude?) { false }
                    expect(subject.out?).to be_falsey
                end
            end
        end
    ...
    describe '#dom_depth_limit_reached?' do
        context "when #{SCNR::Engine::OptionGroups::Scope}#dom_depth_limit has" do
