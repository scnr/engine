...
describe SCNR::Engine::Framework::Parts::Data do
    include_examples 'framework'

    describe '#data' do
        it "returns #{SCNR::Engine::Data::Framework}" do
            expect(subject.data).to be_kind_of SCNR::Engine::Data::Framework
        end
    end
    ...
    describe '#push_to_page_queue' do
        let(:page) { SCNR::Engine::Page.from_url( url + '/train/true' ) }
        ...
        it "passes it to #{SCNR::Engine::ElementFilter}#update_from_page_cache" do
            page = SCNR::Engine::Page.from_url( url + '/link' )

            expect(SCNR::Engine::ElementFilter).to receive(:update_from_page_cache).with(page)
        ...
        context 'when the page has already been seen' do
            it 'ignores it' do
                page = SCNR::Engine::Page.from_url( url + '/stuff' )
        ...
        context "when #{SCNR::Engine::Page::Scope}#out? is true" do
            it 'returns false' do
                allow_any_instance_of(SCNR::Engine::Page::Scope).to receive(:out?) { true }
                expect(subject.push_to_page_queue( page )).to be_falsey
            end
        end

        context "when #{SCNR::Engine::URI::Scope}#redundant? is true" do
            it 'returns false' do
                allow_any_instance_of(SCNR::Engine::Page::Scope).to receive(:redundant?) { true }
                expect(subject.push_to_page_queue( page )).to be_falsey
            end
        end

        context "when #{SCNR::Engine::Page::Scope}#auto_redundant? is true" do
            it 'returns false' do
                allow_any_instance_of(SCNR::Engine::Page::Scope).to receive(:auto_redundant?) { true }
                expect(subject.push_to_page_queue( page )).to be_falsey
            end
        end
    end
    ...
    describe '#push_to_url_queue' do
        ...
        context "when #{SCNR::Engine::URI::Scope}#out? is true" do
            it 'returns false' do
                allow_any_instance_of(SCNR::Engine::URI::Scope).to receive(:out?) { true }
                expect(subject.push_to_url_queue( url )).to be_falsey
            end
        end
        ...
        context "when #{SCNR::Engine::URI::Scope}#redundant? is true" do
            it 'returns false' do
                allow_any_instance_of(SCNR::Engine::URI::Scope).to receive(:redundant?) { true }
                expect(subject.push_to_url_queue( url )).to be_falsey
            end
        end

        context "when #{SCNR::Engine::URI::Scope}#auto_redundant? is true" do
            it 'returns false' do
                allow_any_instance_of(SCNR::Engine::URI::Scope).to receive(:auto_redundant?) { true }
                expect(subject.push_to_url_queue( url )).to be_falsey
            end
        end
    end
