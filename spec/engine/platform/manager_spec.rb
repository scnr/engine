...
describe SCNR::Engine::Platform::Manager do
    ...
    def page
        ...
        page = SCNR::Engine::Page.from_url( "#{web_server_url_for( :auditor )}/s.php" )
    ...
    def binary_page
        SCNR::Engine::Options.do_not_fingerprint
        page = SCNR::Engine::Page.from_url( "#{web_server_url_for( :auditor )}/binary" )
        SCNR::Engine::Options.fingerprint
        page
    end
                        ...
                        context 'and it is text based' do
                            context 'and has not yet been fingerprinted' do
                                ...
                                context 'and is out of scope' do
                                    it 'returns false' do
                                        SCNR::Engine::Options.scope.exclude_path_patterns << /s/
                                        expect(described_class.fingerprint?( page )).to be_falsey
                                    end
                                end
                            end
    ...
    describe '.fingerprint' do
        ...
        context 'even when no platforms have been identified' do
            it 'marks the page as fingerprinted' do
                page = SCNR::Engine::Page.from_url( web_server_url_for( :auditor ) )
    ...
    describe '.[]=' do
        ...
        it 'includes Options.platforms' do
            SCNR::Engine::Options.platforms = [:ruby, :windows]
            platforms = [:unix, :java]

            described_class['http://stuff.com'] = platforms

            expect(described_class['http://stuff.com'].sort).to eq(
                (SCNR::Engine::Options.platforms | platforms).sort
            )
        end

        context 'when invalid platforms are given' do
            it 'raises Engine::Platform::Error::Invalid' do
                expect {
                    described_class['http://stuff.com'] = [:stuff]
                }.to raise_error SCNR::Engine::Platform::Error::Invalid
            end
        end
    end
    ...
    describe '#new_from_options' do
        it 'includes Options.platforms' do
            ...
            expect(described_class.new_from_options( platforms ).sort).to eq(
                (platforms | SCNR::Engine::Options.platforms).sort
            )
        end
    end
    ...
    describe '#os' do
        it 'returns the operating system list' do
            expect(platforms.os).to be_kind_of SCNR::Engine::Platform::List
        end
    end

    describe '#db' do
        it 'returns the database list' do
            expect(platforms.db).to be_kind_of SCNR::Engine::Platform::List
        end
    end

    describe '#servers' do
        it 'returns the server list' do
            expect(platforms.servers).to be_kind_of SCNR::Engine::Platform::List
    ...
    describe '#languages' do
        it 'returns the language list' do
            expect(platforms.languages).to be_kind_of SCNR::Engine::Platform::List
    ...
    describe '#frameworks' do
        it 'returns the framework list' do
            expect(platforms.frameworks).to be_kind_of SCNR::Engine::Platform::List
    ...
    describe '#pick' do
        ...
        context 'when invalid platforms are given' do
            it 'raises Engine::Platforms::Error::Invalid' do
                expect {
