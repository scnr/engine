...
STRINGS = {
    ...
    windows: 'type %SystemDrive%\\\\boot.ini &rem',
}
...
def nested_cookie( n, ns )
    return if !env['HTTP_COOKIE']

    c = SCNR::Engine::NestedCookie.parse_http_cookie( env['HTTP_COOKIE'] )[n]
    return if !c
...
STRINGS.keys.each do |platform|
    ...
    get "/#{platform}/nested_cookie/straight" do
        ...
        value = nested_cookie( 'nested_cookie', 'name' ).to_s
    ...
    get "/#{platform_str}/json" do
        <<-EOHTML
            <script type="application/javascript">
                ...
                http_request.send( '{"input": "scnr_engine_user"}' );
