Factory.define :vector do |type = :Form|
    SCNR::Engine::Element.const_get( type ).new( url: 'http://test.com', inputs: { stuff: 1 } )
end
...
Factory.define :body_vector do
    SCNR::Engine::Element::Body.new Factory[:page].url
end

Factory.define :server_vector do
    SCNR::Engine::Element::Server.new Factory[:response].url
end

Factory.define :path_vector do
    SCNR::Engine::Element::Path.new Factory[:response].url
end
