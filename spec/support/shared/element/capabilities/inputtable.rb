shared_examples_for 'inputtable' do |options = {}|
    ...
    describe '#raw_inputs=' do
        ...
        context 'when a name contains invalid data' do
            it "raises #{SCNR::Engine::Element::Capabilities::Inputtable::Error::InvalidData::Name}" do
                allow(subject).to receive(:valid_input_data?) { |data| data != valid_key }

                expect do
                    subject.raw_inputs = [ valid_key ]
                end.to raise_error SCNR::Engine::Element::Capabilities::Inputtable::Error::InvalidData::Name
            end
        end
    ...
    describe '#inputs=' do
        ...
        it 'converts all inputs to strings',
           if: described_class != SCNR::Engine::Element::JSON do
        ...
        context 'when the input value' do
            context 'contains invalid data' do
                it "raises #{SCNR::Engine::Element::Capabilities::Inputtable::Error::InvalidData::Value}" do
                    allow(subject).to receive(:valid_input_data?) { |data| data != 'blah' }

                    expect do
                        subject.inputs = { valid_key => 'blah' }
                    end.to raise_error SCNR::Engine::Element::Capabilities::Inputtable::Error::InvalidData::Value
                end
            end
