shared_examples_for 'mutable' do |options = {}|
    ...
    it "supports #{SCNR::Engine::RPC::Serializer}" do
        expect(mutation).to eq(SCNR::Engine::RPC::Serializer.deep_clone( mutation ))
    end
    ...
    describe '.from_rpc_data' do
        ...
        let(:data) { SCNR::Engine::RPC::Serializer.rpc_data( mutation ) }
    ...
    describe '#mutations' do
        ...
        context 'with option' do
            describe ':parameter_values' do
                ...
                describe "#{SCNR::Engine::OptionGroups::Audit}#parameter_values" do
                    it 'serves as the default value of :parameter_values' do
                        SCNR::Engine::Options.audit.parameter_values = true
                        expect(mutable.mutations( seed ).
                            find { |m| m.affected_input_value.include? seed }).
                            to be_truthy

                        SCNR::Engine::Options.audit.parameter_values = false
                        expect(mutable.mutations( seed ).
                            find { |m| m.affected_input_value.include? seed }).
                            to be_falsey
                    end
            ...
            describe ':with_raw_payloads',
                     if: !described_class.ancestors.include?(
                         SCNR::Engine::Element::DOM
                     ) && described_class != SCNR::Engine::Element::JSON &&
                             described_class != SCNR::Engine::Element::XML &&
                             described_class != SCNR::Engine::Element::Header &&
                             described_class != SCNR::Engine::Element::Cookie do
                ...
                describe "#{SCNR::Engine::OptionGroups::Audit}#with_raw_payloads" do
                    it 'serves as the default value of :with_raw_payloads' do
                        SCNR::Engine::Options.audit.with_raw_payloads = true
                        expect(
                            mutable.mutations( seed ).find(&:with_raw_payload?)
                        ).to be_truthy

                        SCNR::Engine::Options.audit.with_raw_payloads = false
                        expect(
                            mutable.mutations( seed ).find(&:with_raw_payload?)
                        ).to be_falsey
                    end
            ...
            describe 'with_extra_parameter',
                     if: !described_class.ancestors.include?(
                         SCNR::Engine::Element::DOM
                     ) && described_class != SCNR::Engine::Element::LinkTemplate &&
                             described_class != SCNR::Engine::Element::XML do
                ...
                describe "#{SCNR::Engine::OptionGroups::Audit}#with_extra_parameter" do
                    it 'serves as the default value of :with_extra_parameter' do
                        SCNR::Engine::Options.audit.with_extra_parameter = true
                        expect(mutable.mutations( seed ).
                            find { |m| m[extra_name].to_s.include? seed }).to be_truthy

                        SCNR::Engine::Options.audit.with_extra_parameter = false
                        expect(mutable.mutations( seed ).
                            find { |m| m[extra_name].to_s.include? seed }).to be_falsey
                    end
                end
            ...
            describe 'with_both_http_methods',
                ...
                describe "#{SCNR::Engine::OptionGroups::Audit}#with_both_http_methods" do
                    it 'serves as the default value of :with_both_http_methods' do
                        SCNR::Engine::Options.audit.with_both_http_methods = true
                        expect(mutable.mutations( seed ).map(&:method).uniq).
                            to eq [:get, :post]

                        SCNR::Engine::Options.audit.with_both_http_methods = false
                        expect(mutable.mutations( seed ).map(&:method).uniq).
                            to eq([mutable.method])
                    end
                end
            ...
            describe 'parameter_names',
                ...
                describe "#{SCNR::Engine::OptionGroups::Audit}#parameter_names" do
                    it 'serves as the default value of :parameter_names' do
                        SCNR::Engine::Options.audit.parameter_names = true
                        expect(mutable.mutations( seed ).
                            find { |m| m.inputs.keys.include? seed }).
                            to be_truthy

                        SCNR::Engine::Options.audit.parameter_names = false
                        expect(mutable.mutations( seed ).
                            find { |m| m.inputs.keys.include? seed }).
                            to be_falsey
                    end
            ...
            describe ':format' do
                ...
                describe 'Format::APPEND' do
                    it 'appends the seed to the current value' do
                        m = mutable.mutations( seed,
                                                format: [SCNR::Engine::Element::Capabilities::Mutable::Format::APPEND],
                                                skip_original: true ).first
                ...
                describe 'Format::NULL' do
                    it 'terminates the string with a null character',
                       if: described_class != SCNR::Engine::Element::Header &&
                               described_class.is_a?( SCNR::Engine::Element::DOM ) do

                        m = mutable.mutations( seed,
                                                format: [SCNR::Engine::Element::Capabilities::Mutable::Format::NULL],
                                                skip_original: true ).first
                ...
                describe 'Format::SEMICOLON' do
                    it 'prepends the seed with a semicolon' do
                        m = mutable.mutations( seed,
                                                format: [SCNR::Engine::Element::Capabilities::Mutable::Format::SEMICOLON],
                                                skip_original: true ).first
                        expect(m[m.affected_input_name]).to eq(';' + seed)
                    end
                end
                describe 'Format::APPEND | Format::NULL' do
                    it 'appends the seed and terminate the string with a null character',
                       if: described_class != SCNR::Engine::Element::Header &&
                        ...
                        format = [SCNR::Engine::Element::Capabilities::Mutable::Format::APPEND |
                                      SCNR::Engine::Element::Capabilities::Mutable::Format::NULL]
        ...
        context 'when there is an #auditor' do
            context "and its #skip? method returns true" do
                it 'skips the mutation'
            end

            context "and its #skip? method returns false" do
                it 'allows the mutation'
            end
        end
