shared_examples_for 'auditable'  do |options = {}|
    ...
    it "supports #{SCNR::Engine::RPC::Serializer}" do
        expect(auditable).to eq(SCNR::Engine::RPC::Serializer.deep_clone( auditable ))
    end
    ...
    describe '#audit' do
            ...
            context 'Hash' do
                ...
                context 'and is empty' do
                    it 'returns nil' do
                        injected = []
                        expect(auditable.audit( {},
                                          format: [ SCNR::Engine::Check::Auditor::Format::STRAIGHT ] ) do |_, element|
                            injected << element.affected_input_value
                        end).to be_nil
        ...
        context 'when called with option' do
            describe ':submit' do
                it 'uses them for the #submit call' do
                    ...
                    each = proc do |mutation|
                        expect(mutation).to receive(:submit).with(options.merge( data_flow_taint: mutation.affected_input_value ))
                        called = true
                    end
            ...
            describe ':each_mutation' do
                it 'is passed each generated mutation' do
                    ...
                    auditable.audit( seed, each_mutation: each_mutation,
                                      skip_original: true,
                                      format: [ SCNR::Engine::Check::Auditor::Format::STRAIGHT ] ) do |res, _|
            ...
            describe ':format' do
                describe 'Engine::Check::Auditor::Format::STRAIGHT' do
                    it 'injects the seed as is' do
                        ...
                        auditable.audit( seed,
                                          skip_original: true,
                                          format: [ SCNR::Engine::Check::Auditor::Format::STRAIGHT ] ) do |res, e|
                ...
                describe 'Engine::Check::Auditor::Format::APPEND' do
                    it 'appends the seed to the existing value of the input' do
                        ...
                        auditable.audit( seed,
                                          skip_original: true,
                                          format: [ SCNR::Engine::Check::Auditor::Format::APPEND ] ) do |res, e|
                            injected = auditable_extract_parameters( res )[e.affected_input_name]
                            cnt += 1
                        end
                ...
                describe 'Engine::Check::Auditor::Format::NULL' do
                    it 'terminates the seed with a null character',
                       if: described_class != SCNR::Engine::Element::Header &&
                            described_class != SCNR::Engine::Element::XML &&
                               !described_class.ancestors.include?( SCNR::Engine::Element::DOM ) do
                        ...
                        auditable.audit( seed,
                                          skip_original: true,
                                          format: [ SCNR::Engine::Check::Auditor::Format::NULL ] ) do |res, e|
                            injected = auditable_extract_parameters( res )[e.affected_input_name]
                            cnt += 1
                        end
                ...
                describe 'Engine::Check::Auditor::Format::SEMICOLON' do
                    it 'prepends the seed with a semicolon' do
                        ...
                        format = [ SCNR::Engine::Check::Auditor::Format::SEMICOLON ]
        ...
        context "when the #{SCNR::Engine::OptionGroups::Audit}#exclude_vector_patterns option is set" do
            it 'skips those vectors by name' do
                SCNR::Engine::Options.audit.exclude_vector_patterns = auditable.inputs.keys
        ...
        context "when #{SCNR::Engine::OptionGroups::Audit}#vector?" do
            context 'returns true' do
                it 'audits the input' do
                    allow(SCNR::Engine::Options.audit).to receive(:vector?){ true }
            ...
            context 'returns false' do
                it 'skips the input' do
                    allow(SCNR::Engine::Options.audit).to receive(:vector?){ false }
