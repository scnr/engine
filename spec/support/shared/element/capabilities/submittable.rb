shared_examples_for 'submittable' do
    ...
    rpc_attributes = if [SCNR::Engine::Element::Link::DOM,
                         SCNR::Engine::Element::LinkTemplate::DOM].include? described_class
                         %w(url method)
                     else
                         %w(url action method)
                     end
    ...
    describe '.from_rpc_data' do
        ...
        let(:data) { SCNR::Engine::RPC::Serializer.rpc_data( submittable ) }
    ...
    describe '#platforms' do
        it 'returns platforms for the given element' do
            expect(submittable.platforms).to be_kind_of SCNR::Engine::Platform::Manager
        end
    end
