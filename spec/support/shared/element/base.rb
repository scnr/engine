...
shared_examples_for 'element' do
    ...
    let :normalized_url do
        SCNR::Engine::Utilities.normalize_url( 'http://test.com' )
    end
    let(:page) { defined?( super ) ? super() : Factory[:page].dup }

    it "supports #{SCNR::Engine::RPC::Serializer}" do
        expect(subject).to eq(SCNR::Engine::RPC::Serializer.deep_clone( subject ))
    end

    rpc_attributes = if described_class.ancestors.include? SCNR::Engine::Element::DOM
                         %w(url)
                     else
    ...
    describe '#url=',
        ...
        it 'normalizes the passed URL' do
            ...
            subject.url = url
            expect(subject.url).to eq(SCNR::Engine::Utilities.normalize_url( url ))
        end
    end
