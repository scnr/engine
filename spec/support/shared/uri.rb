...
shared_examples_for 'uri' do
    ...
    before( :all ) do
        @urls = [
            ...
            'http://192.168.0.232/dvwa/phpinfo.php?=PHPB8B5F2A0-3C92-11d3-A3A9-4C7B08C10000%23%5E%28%24%21%40%24%29%28%28%29%29%29%2A%2A%2A%2A%2A%2A&_scnr_engine_trainer_c987fdb6d3955bd60191449bc465bb5ca760f60661fa4bcdf28736ae04aa2a1e=c987fdb6d3955bd60191449bc465bb5ca760f60661fa4bcdf28736ae04aa2a1e',
        ...
        @normalized = {
           ...
           "http://test.com/stuff?name=val&amp;name2=val2"=>
               "http://test.com/stuff?name=val&name2=val2",
           "http://testfire.net/bank/queryxpath.aspx?__EVENTVALIDATION=%2FwEWAwLNx%2B2YBwKw59eKCgKcjoPABw%3D%3D&__VIEWSTATE=%2FwEPDwUKMTEzMDczNTAxOWRk&_ctl0%3A_ctl0%3AContent%3AMain%3AButton1=Query&_ctl0%3A_ctl0%3AContent%3AMain%3ATextBox1=Enter+title+%28e.g.+IBM%29%27%3Becho+287630581954%2B4196403186331128%3B%23"=>
                "http://testfire.net/bank/queryxpath.aspx?__EVENTVALIDATION=/wEWAwLNx%2B2YBwKw59eKCgKcjoPABw==&__VIEWSTATE=/wEPDwUKMTEzMDczNTAxOWRk&_ctl0:_ctl0:Content:Main:Button1=Query&_ctl0:_ctl0:Content:Main:TextBox1=Enter%20title%20(e.g.%20IBM)'%3Becho%20287630581954%2B4196403186331128%3B%23",
           "http://192.168.0.232/dvwa/phpinfo.php?=PHPB8B5F2A0-3C92-11d3-A3A9-4C7B08C10000%23%5E%28%24%21%40%24%29%28%28%29%29%29%2A%2A%2A%2A%2A%2A&_scnr_engine_trainer_c987fdb6d3955bd60191449bc465bb5ca760f60661fa4bcdf28736ae04aa2a1e=c987fdb6d3955bd60191449bc465bb5ca760f60661fa4bcdf28736ae04aa2a1e"=>
               "http://192.168.0.232/dvwa/phpinfo.php?=PHPB8B5F2A0-3C92-11d3-A3A9-4C7B08C10000%23%5E($!@$)(()))******&_scnr_engine_trainer_c987fdb6d3955bd60191449bc465bb5ca760f60661fa4bcdf28736ae04aa2a1e=c987fdb6d3955bd60191449bc465bb5ca760f60661fa4bcdf28736ae04aa2a1e",
           "http://foo.com/user/login?user%5Bname%5D=bar&user%5Bpass%5D=asdasd%26asdihbasd"=>
               "http://foo.com/user/login?user%5Bname%5D=bar&user%5Bpass%5D=asdasd%26asdihbasd",
           "http://stuff.host.fdfd/web/seguros/auto;jsessionid=6CB5A6A4597FFFA80C4D23B235072588.000?test=tet"=>
    ...
    before(:each) do
        @opts = SCNR::Engine::Options.instance.reset
        described_class::CACHE.values.each(&:clear)
    end
    ...
    describe '.rewrite' do
        ...
        context 'when no rules are provided' do
            it "uses the ones in #{SCNR::Engine::OptionGroups::Scope}#url_rewrites" do
                SCNR::Engine::Options.scope.url_rewrites = rewrite_rules
    ...
    describe '.decode' do
        it 'decodes a URI' do
            uri = 'my%20test.asp?name=st%C3%A5le&car=saab+1'
            expect(described_class.decode( uri )).to eq("my test.asp?name=ståle&car=saab 1")
        end
    end
...
9TXL0Y4OHwAAAABJRU5ErkJggg==' )).to be_nil
    ...
    describe '#==' do
        it 'converts both objects to strings and compare them' do
            @urls.each do |url|
                ...
                expect(a_uri.is_a?( described_class )).to be_truthy

                expect(a_uri.to_s).to eq(normalized_str)
    ...
    describe '#seed_in_host?' do
        ...
        context 'when the seed is in the domain' do
            let(:url) { "http://www.#{SCNR::Engine::Utilities.random_seed}.com/stuff" }
        ...
        context 'when the seed is in the subdomain' do
            let(:url) { "http://#{SCNR::Engine::Utilities.random_seed}.test.com" }
        ...
        context 'when the seed is in the TLD' do
            let(:url) { "http://test.#{SCNR::Engine::Utilities.random_seed}" }
    ...
    describe '#host=' do
        ...
        it 'sets the #host' do
            subject.host = 'stuff'
            expect(subject.host).to eq 'stuff'
        end
    ...
    describe '#userinfo=' do
        ...
        it 'sets the #userinfo' do
            subject.userinfo = 'user:pass'
            expect(subject.userinfo).to eq 'user:pass'
        end

        context 'when given an empty string' do
            it 'removes it' do
                subject.userinfo = ''
                expect(subject.userinfo).to be_nil
            end
        end

        context 'when given nil' do
            it 'removes it' do
                subject.userinfo = nil
                expect(subject.userinfo).to be_nil
            end
        end
    end
    ...
    describe '#dup' do
        subject { described_class.new( 'http://user:pass@test.com:4567/?my=val' ) }
    ...
    describe '#to_absolute' do
        it 'converts a self to absolute using the reference URL' do
            ...
            rel  = '/test'
            rel2 = 'test2'
            expect(described_class.parse( rel ).to_absolute( abs ).to_s).to eq("http://test.com" + rel)
            expect(described_class.parse( rel2 ).to_absolute( abs ).to_s).to eq("http://test.com/blah/" + rel2)
            expect(described_class.parse( rel2 ).to_absolute( abs + '/' ).to_s).to eq("http://test.com/blah/ha/" + rel2)
        end
    end
    ...
    describe '#domain' do
        ...
        context 'when host is an IP address' do
            it 'returns the host' do
                url = 'http://127.0.0.1/'
                expect(described_class.parse( url ).domain).to eq('127.0.0.1')
            end
        end
    ...
    describe '#query_parameters' do
        it 'parses the query parameters' do
            params = described_class.parse( 'http://test.com/?1%20e=2&3=d%204&=5&6' ).query_parameters
            expect(params).to eq({
                '1 e' => '2',
                '3'   => 'd 4',
                ''    => '5',
