...
class Manager
    ...
    def kill( pid )
        ...
        Timeout.timeout 10 do
            while sleep 0.1 do
                begin
                    Process.kill( SCNR::Engine.windows? ? 'KILL' : 'TERM', pid )
    ...
    #   PIDs of the process to {SCNR::Engine::Processes::Manager#kill}.
    def kill_many( pids )
    ...
    def spawn( executable, options = {} )
        ...
        engine_options = Options.dup.update( options.delete(:options) || {} ).to_h
        encoded_engine_options = Base64.strict_encode64( Marshal.dump( engine_options ) )
        ...
        pid = Process.spawn(
            {
                'SCNR_ENGINE_SPAWN_OPTIONS' => encoded_engine_options
            },
            RbConfig.ruby,
            RUNNER,
            *(argv + [spawn_options])
