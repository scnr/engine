...
class Session
    ...
    class Error < SCNR::Engine::Error
        ...
        # Raised when trying to {Session#login} without proper
        ...
        find = proc do |cforms|
            cforms.select do |f|
                ...
                if action = opts[:action]
                    oks << (action.is_a?( Regexp ) ?
                        action.match?( f.action ) : f.action == action)
                end
    ...
    def login_from_configuration
        ...
        if has_browser?
            # Use the form DOM to submit if a browser is available even if the
            # form doesn't use JS.
            form.skip_dom = false
            form = form.dom
            form.browser = browser

            if !browser.javascript.dom_monitor.is_visible_selector( form.locator.css )
        ...
        if has_browser?
            ...
            if click_button
                ...
                transitions = []
                transitions << browser.fire_event( form.locator, :fill, inputs: form.inputs )
    ...
    def refresh_browser
        ...
        @browser = Browser.new(
            store_pages:   false,
            ignore_scope:  true,
            parse_profile: Browser::ParseProfile.only( :body )
        )
    end
