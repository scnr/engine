...
# State information for {SCNR::Engine::BrowserCluster}.
#
...
class BrowserCluster
    ...
    def self.load( directory )
        ...
        %w(pending_jobs).each do |attribute|
            ...
            browser_cluster.send(attribute).merge! Marshal.load( IO.binread( path ) )
        end

        Marshal.load( IO.binread( "#{directory}/job_callbacks" ) ).each do |id, m|
            browser_cluster.job_callbacks[id] = m.first.method( m.last )
        end
        ...
        %w(pending_job_counter job_id total_job_time time_out_count
            ...
            browser_cluster.send( "#{attribute}=", Marshal.load( IO.binread( path ) ) )
        end
