...
var _tokenDOMMonitor = {
    ...
    exclude_attributes_from_digest:  ['data-scnr-engine-id'],

    exclude_elements_with_attribute_from_digest:  'data-scnr-engine-no-digest',
    ...
    initialize: function ( event_inheritance_limit ) {
        ...
        _tokenDOMMonitor.accelerate_requestAnimationFrame();
    ...
    elements_with_events: function ( offset, batch_size, max_events, tag_name_whitelist ) {
        ...
        var elements_with_events = [];
        var elements = document.getElementsByTagName("*");
        var length   = elements.length;

        var global_events = window._scnr_engine_events || [];
        global_events = global_events.concat( document._scnr_engine_events || [] );
        ...
        for( var i = 0; i < length; i++ ) {
            ...
            var e = {
                tag_name:   tag_name,
                events:     element._scnr_engine_events || [],
                attributes: {}
            };
            ...
            if(
                ...
                e.events = e.events.concat( element._scnr_engine_inherited_events || [] );
            ...
            elements_with_events.push( e );

            if( max_events ) {
                event_counter += Object.keys( e.events ).length;

                if( event_counter >= max_events ){
                    return elements_with_events;
                }
            }

            // Batch size reached, send it on its way.
            if( batch_size && elements_with_events.length == batch_size )
                return elements_with_events;
        }
    ...
    // Returns an Integer digest of summary().
    digest: function () {
        return _tokenDOMMonitor.hashCode( _tokenDOMMonitor.summary() );
    },

    // Returns a String summary of the current DOM tree (i.e. node names and
    ...
    summary: function () {
        ...
        for( var i = 0; i < length; i++ ) {
            ...
            if( element.getAttribute(
                    _tokenDOMMonitor.exclude_elements_with_attribute_from_digest
                )
            )
                continue;
            ...
            for( var j = 0; j < attr_length; j++ ){
                ...
                summary += ' ' + attributes[j].nodeName + '=' + attributes[j].nodeValue;
            }
            summary += '>'
        }
    ...
    //
    // DO NOT execute i,immediately as an optimization, nested timeouts to
    // simulate controlled intervals could lead to a continuous inf loop that'll
    // make the browser hit 100% CPU and in general go nuts.
    //
    // TODO: Maybe wrap the callback to send an AJAX call to signal actual firing?
    track_setTimeout: function () {
        ...
        window.setTimeout = function() {
            ...
            // arguments[1] = 0;
            return original_setTimeout.apply( this, arguments );
        };
    },
    ...
    track_setInterval: function () {
        ...
        window.setInterval = function() {
            ...
            _tokenDOMMonitor.intervals.push( args );

            // arguments[1] = 0;
            return original_setInterval.apply( this, arguments );
        };
    },

    accelerate_requestAnimationFrame: function () {
        var original_requestAnimationFrame = window.requestAnimationFrame;

        window.requestAnimationFrame = function() {
            return original_requestAnimationFrame.apply( this, Date.now() + 999999999999 );
        }
    },
    ...
    bequeath_events: function( element ) {
        ...
        for( var i = 0; i < children.length; i++ ) {
            ...
            if( !('_scnr_engine_inherited_events' in child) ) child['_scnr_engine_inherited_events'] = [];

            // Merge the element's events with the child's existing inherited ones.
            if( element['_scnr_engine_events'] ) {
                child['_scnr_engine_inherited_events'] =
                    element['_scnr_engine_events'].concat( child['_scnr_engine_inherited_events'] );
            }

            // Merge the element's inherited events with the child's existing
            // inherited ones.
            if( element['_scnr_engine_inherited_events'] ) {
                child['_scnr_engine_inherited_events'] =
                    element['_scnr_engine_inherited_events'].concat( child['_scnr_engine_inherited_events'] );
            }

            // Make sure we didn't add duplicates.
            child['_scnr_engine_inherited_events'] =
                _tokenDOMMonitor.arrayUnique( child['_scnr_engine_inherited_events'] )
        }
    },
    ...
    registerEvent: function ( element, event, handler ) {
        if( !('_scnr_engine_events' in element) ) element['_scnr_engine_events'] = [];
        ...
        element['_scnr_engine_events'].push( [event, handler.toString()] );
    },
    ...
    setElementIds: function() {
        ...
        for( var i = 0; i < length; i++ ) {
            ...
            if( !element._scnr_engine_events || element._scnr_engine_events.length == 0 ) continue;

            element.setAttribute( 'data-scnr-engine-id', _tokenDOMMonitor.hashCode( element.innerHTML ) );
        }
    },

    element_input_names: function ( selector ) {
        var inputs = _tokenDOMMonitor.element_inputs( selector );
        if( !inputs ) return false;

        return Object.keys( inputs );
    },

    element_inputs: function ( selector ) {
        var element = document.querySelector( selector );
        if( !element ) return false;

        var inputs         = {};
        var input_elements = element.querySelectorAll( 'input, textarea, select' );
        for( var i = 0; i < input_elements.length; i++ ) {
            inputs[
                input_elements[i].getAttribute( 'name' ) ||
                    input_elements[i].getAttribute( 'id' )
            ] = input_elements[i].getAttribute( 'value' );
        }

        return inputs;
    },

    does_element_have_events: function( selector ) {
        var element = document.querySelector( selector );
        if( !element ) return [];

        return element._scnr_engine_events ? !!element._scnr_engine_events.length : false;
    },
    ...
    is_visible_selector: function( selector ) {
        var element = document.querySelector( selector );
        if( !element ) return false;

        return _tokenDOMMonitor.is_visible( element );
    },
