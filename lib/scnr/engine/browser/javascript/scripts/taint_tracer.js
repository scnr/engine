...
var _tokenTaintTracer = {
    ...
    data_flow_sinks_to_monitor: {
        ...
        once: [
            [
                ...
                [
                    'after', 'before', 'remove', 'replaceWith', 'insertData',
            ...
            [Document.prototype, ['createTextNode'], 'Document'],
            [HTMLDocument.prototype, ['write', 'writeln'], 'HTMLDocument'],
            [Element.prototype, ['setAttribute'], 'Element'],
            [HTMLElement.prototype, ['insertAdjacentHTML'], 'HTMLElement'],
            [
                String.prototype,
                ['replace', 'concat', 'indexOf', 'lastIndexOf'],
                'String'
            ],
            [
    ...
    initialize: function ( taints ) {
        ...
        if( !_tokenTaintTracer.isEmpty( taints ) ) {
            _tokenTaintTracer.install_tracers_from_list(
                _tokenTaintTracer.data_flow_sinks_to_monitor.once
            );
        }
    ...
    update: function () {
        _tokenTaintTracer.install_tracers_from_list(
            _tokenTaintTracer.data_flow_sinks_to_monitor.watch
        );
    },
    ...
    log_data_flow_sink: function ( taint, frame_data ){
        ...
        // In case the page redirects at least get a signal.
        //
        // TODO: Look into making the request sync.
        //  Also, setup a management service thingy on the other side to
        //  catch and handle signals in general, like:
        //      http://signals.browser.scnr.engine/
        //  Doesn't need to be AJAX, iframe, image/script src, whatever works.
        //
        //
        // NOTE: This will be traced, find a way around it.
        if( _tokenTaintTracer.data_flow_sinks[taint].length === 1 ) {
            var http = new XMLHttpRequest();
            http.open( "GET", "/data_flow_sink_signal/" );
            http.send();
        }
    ...
    cleanup_event: function( e ) {
        ...
        for( var i = 0; i < keep.length; i++ ) {
            ...
            // Dealing with a HTMLDocument, needs some special treatment to get
            // the HTML.
...
var _token_taint_tracer = _tokenTaintTracer;
