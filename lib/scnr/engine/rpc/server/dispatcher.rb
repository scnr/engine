...
class Dispatcher
    ...
    def dispatch( owner = 'dispatcher', helpers = {}, load_balance = true, &block )
        if load_balance && @node.grid_member?
            preferred do |url|
                ...
                connect_to_peer( url ).dispatch(
                    owner,
                    helpers.merge( via: @url ),
                    false,
                    &block
                )
            end
    ...
    def instance( pid )
        @instances.each do |i|
            ...
            i['alive'] = SCNR::Engine::Processes::Manager.alive?( pid )
