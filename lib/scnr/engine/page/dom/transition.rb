...
class Transition
    ...
    def play( browser )
        ...
        t = browser.fire_event element, event, options

        post_windows = browser.selenium.window_handles

        return t if pre_windows_size == post_windows.size

        begin
            browser.print_debug 'Switching to new window.'

            # Transition could have opened a new window and we need to switch to
            # it to maintain the proper workflow.
            browser.selenium.switch_to.window( post_windows.last )
        rescue => e
            browser.print_debug 'Could not switch to new window.'
            browser.print_debug
            browser.print_debug_exception e
