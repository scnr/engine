...
class Report
    ...
    #   Location for the {#to_ser dumped} report file.
    #
    # @return   [String]
    ...
    # @return   [String]
    #   Report serialized in the SCNR::Engine Report format.
    def to_ser
        ser = RPC::Serializer.dump( self )

        # Append metadata to the end of the dump.
        metadata = RPC::Serializer.dump( summary )
        ser << [metadata, metadata.size].pack( 'a*N' )
    ...
    def to_h
        h = {
            ...
            status:          @status,
            seed:            @seed,
            options:         SCNR::Engine::Options.hash_to_rpc_data( @options ),
            sitemap:         @sitemap,
    ...
    def to_rpc_data
        ...
        data['options'] = SCNR::Engine::Options.hash_to_rpc_data( data['options'] )
    ...
    def self.from_rpc_data( data )
        ...
        data['issues'] = data['issues'].map { |i| SCNR::Engine::Issue.from_rpc_data( i ) }
    ...
    def prepare_options( options )
        SCNR::Engine::Options.rpc_data_to_hash( options.to_rpc_data_or_self )
    end
