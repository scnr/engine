...
#
# * {Engine::Element::Capabilities::Analyzable::Signature Signature analysis}
#   -- {#audit}
# * {Engine::Element::Capabilities::Analyzable::Timeout Timeout analysis}
#   -- {#audit_timeout}
# * {Engine::Element::Capabilities::Analyzable::Differential Differential analysis}
#   -- {#audit_differential}
#
# It should be noted that actual analysis takes place at the {Engine::Element element} level.
#
# It also provides:
#
...
#   * {#match_and_log}
# * General {Engine::Issue} logging helpers.
#   * {#log}
#   * {#log_issue}
#
...
module Auditor
    ...
    module ClassMethods
        def calculate_audit_cost( *args )
            Element::Capabilities::Auditable.calculate_cost *args
        end

        def calculate_signature_analysis_cost( *args )
            Element::Capabilities::Analyzable::Signature.calculate_cost *args
        end

        def calculate_differential_analysis_cost( *args )
            Element::Capabilities::Analyzable::Differential.calculate_cost *args
        end

        def calculate_timeout_analysis_cost( *args )
            Element::Capabilities::Analyzable::Timeout.calculate_cost *args
        end
        ...
        #
        # @return   [Bool]
        def check?( page, restrict_to_elements = nil, ignore_dom_depth = false )
            return false if issue_limit_reached?
            return true  if elements.empty?

            audit                = SCNR::Engine::Options.audit
    ...
    FILE_SIGNATURES_PER_PLATFORM =
        SCNR::Engine::Element::Capabilities::Analyzable::Signature::FILE_SIGNATURES_PER_PLATFORM

    FILE_SIGNATURES =
        SCNR::Engine::Element::Capabilities::Analyzable::Signature::FILE_SIGNATURES

    SOURCE_CODE_SIGNATURES_PER_PLATFORM =
        SCNR::Engine::Element::Capabilities::Analyzable::Signature::SOURCE_CODE_SIGNATURES_PER_PLATFORM
    ...
    ELEMENTS_WITH_INPUTS = [
        ...
        Element::NestedCookie, Element::LinkTemplate, Element::JSON, Element::XML
    ]
    ...
    # @see Engine::Check::Base#preferred
    # @see Engine::Check::Base.prefer
    # @abstract
    def preferred
        []
    end

    # This is called right before an {Engine::Element} is audited and is used
    # to determine whether to skip it or not.
    #
    # Running checks can override this as they wish *but* at their own peril.
    #
    # @param    [SCNR::Engine::Element]  element
    #
    # @return   [Boolean]
    ...
    def skip?( element )
        ...
        # Don't check sink compatibility for form mutations with original or
        # sample values.
        if !element.is_a?( Element::Form ) ||
            (
                !element.mutation_with_original_values? &&
                    !element.mutation_with_sample_values?
            )

            return true if !compatible_sinks?( element )
        end
    ...
    def compatible_sinks?( element )
        ...
        return true if !element.respond_to?( :sinks )

        # Check does not care about sinks, allow all elements.
        #
        # If no checks that need sink info have been loaded then there would
        # have been no reason to run the trace.
        return true  if !self.class.has_sinks?

        # For some reason this mutation gets a pass, probably because the cost
        # of tracing it is higher than the cost of auditing it.
        return true  if element.sinks.override?

        # Sinks for this element have not been traced yet, we'll catch it at
        # a later pass.
        return false if !element.sinks.traced?

        self.class.supports_sinks?( element.sinks )
    end
    ...
    # If a block has been provided it calls {Engine::Element::Capabilities::Auditable#audit}
    # for every element, otherwise, it defaults to {#audit_signature}.
    #
    # Uses {#each_candidate_element} to decide which elements to audit.
    #
    # @see Engine::Element::Capabilities::Auditable#audit
    # @see #audit_signature
    ...
    # Uses {#each_candidate_element} to decide which elements to audit.
    #
    # @see Engine::Element::Capabilities::Analyzable::Signature
    ...
    # Uses {#each_candidate_element} to decide which elements to audit.
    #
    # @see Engine::Element::Capabilities::Analyzable::Differential
    ...
    # Uses {#each_candidate_element} to decide which elements to audit.
    #
    # @see Engine::Element::Capabilities::Analyzable::Timeout
