...
class Request < Message
    ...
    ENCODE_CACHE = Support::Cache::LeastRecentlyPushed.new( size: 2_000 )
    ...
    #   Force Engine to analyze the response looking for new elements.
    ...
    def on_complete( &block )
        fail 'Block is missing.' if !block_given?
        @on_complete << profile_wrap_proc( &block )
        self
    end

    def on_body( &block )
        fail 'Block is missing.' if !block_given?
        @on_body << profile_wrap_proc( &block )
        self
    end

    def on_body_line( &block )
        fail 'Block is missing.' if !block_given?
        @on_body_line << profile_wrap_proc( &block )
        self
    end

    def on_body_lines( &block )
        fail 'Block is missing.' if !block_given?
        @on_body_lines << profile_wrap_proc( &block )
        self
    end
    ...
    def to_typhoeus
        ...
        if !userpwd && Options.parsed_url
            if (userpwd = Options.parsed_url.user)
                if (passwd = Options.parsed_url.password)
                    userpwd += ":#{passwd}"
                end
            end
        end
        ...
        options = {
            ...
            # We may occasionally work inside Threads and we need to set this to
            # prevent segfaults.
            nosignal:        true,
