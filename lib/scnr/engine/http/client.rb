...
require_relative 'client/soft_404'
...
class Client
    ...
    #
    #   @return   [SCNR::Engine::HTTP::Client]
    #       `self`
    advertise :after_run
    ...
    #
    # @author Tasos "Zapotek" Laskos <tasos.laskos@gmail.com>
    class Error < SCNR::Engine::HTTP::Error
    end
    ...
    def reset_options
        ...
        headers.merge!(
            ...
            SEED_HEADER_NAME      => SCNR::Engine::Utilities.random_seed
        )
    ...
    def run
        exception_jail false do
            ...
            loop do
                ...
                break if observers_for( :after_run ).empty? && queue_size == 0
            end

            notify_after_each_run

            # Prune the custom 404 cache after **ALL* callbacks have been called,
            # don't prune the cache for emergency runs.
            @soft_404.prune
    ...
    #
    # @see #request
    def get( url = Options.url, options = {}, &block )
    ...
    #
    # @see #request
    def post( url = Options.url, options = {}, &block )
    ...
    #
    # @see #request
    def trace( url = Options.url, options = {}, &block )
    ...
    #
    # @see #request
    def cookie( url = Options.url, options = {}, &block )
    ...
    #
    # @see #request
    def header( url = Options.url, options = {}, &block )
        options[:headers] ||= {}
    ...
    # @param    [Array<String, Hash, SCNR::Engine::Element::Cookie>]   cookies
