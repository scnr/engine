...
class CookieJar
    ...
    # Same as {#initialize}.
    #
    # @return   [SCNR::Engine::HTTP::CookieJar]
    ...
    def update( cookies )
        [cookies].flatten.each do |c|
            ...
            self << case c
                        when String
                            Cookie.from_set_cookie( ::SCNR::Engine::Options.url.to_s, c )

                        when Hash
                            next if c.empty?

                            if c.size > 1
                                Cookie.new( { url: ::SCNR::Engine::Options.url.to_s }.merge( c ) )
                            else
                                Cookie.new( url: ::SCNR::Engine::Options.url.to_s, inputs: c )
                            end
                        when Cookie
                            c
                    end
    ...
    def for_domain( domain )
        self.cookies.select { |c| in_domain?( c.domain, domain ) }
    end
    ...
    def to_uri( url )
        u = url.is_a?( SCNR::Engine::URI ) ? url : SCNR::Engine::URI( url.to_s )
