...
class Slots
    ...
    # 400MB RAM per browser -- need to do something about this.
    BROWSER_SIZE  = 400 * 1024 * 1024

    # 500MB RAM per instance.
    INSTANCE_SIZE = 500 * 1024 * 1024

    # 2.5GB of available disk space per instance.
    DISK_SPACE    = 2.5  * 1024 * 1024 * 1024
    ...
    def memory_size
        # We don't know the settings when the instance is spawned, only after
        # the scan starts, so the best we can do is assume the defaults.
        #
        # Update it in the future to be adjusted after configuration?
        (Options.browser_cluster.pool_size * BROWSER_SIZE) +
            INSTANCE_SIZE
    end
