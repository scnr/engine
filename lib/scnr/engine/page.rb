...
class Page
    ...
    def self.from_data( data )
        ...
        data[:response][:request] = SCNR::Engine::HTTP::Request.new( data[:response][:request] )
        data[:response]           = SCNR::Engine::HTTP::Response.new( data[:response] )
    ...
    # @return   [Integer]
    #   Used to override deduplication using {#hash} and {#persistent_hash},
    #   signifying that a given element has had its DOM sinks traced.
    #
    # @see Element::Capabilities::WithSinks#sink_hash
    # @see Element::DOM::Capabilities::WithSinks#sink_hash
    attr_accessor   :element_sink_trace_hash
    ...
    def has_script?
        ...
        if dbody.optimized_include?( '<script' ) ||
            dbody.optimized_include?( 'javascript:' )
        ...
        Browser::Javascript.events.flatten.each do |event|
            return @has_javascript = true if dbody.optimized_include?( "#{event}=" )
        end

        if Options.audit.element? :links
            self.links.each do |e|
                return @has_javascript = true if e.dom
            end
        end

        if Options.audit.element? :link_templates
            self.link_templates.each do |e|
                return @has_javascript = true if e.dom
            end
        end
    ...
    def has_elements?( *tags )
        ...
        tags.flatten.each do |tag|
            ...
            document.nodes_by_name( tag ) { return true }
        end
    ...
    def title
        ...
        document.nodes_by_name( :title ) { |n| return @title = n.text }
    end
    ...
    def to_initialization_options( deep = true )
        ...
        h[:element_sink_trace_hash] = @element_sink_trace_hash
    ...
    def store_to_metadata( element )
        METADATA.each do |meta|
            ...
            if @metadata[element.type.to_s][meta.to_s][element.coverage_hash].nil?
    ...
    def digest
        ...
        ELEMENTS.each do |type|
            ...
            element_hashes |= list.map(&:persistent_hash)
        end

        # We include the transitions because DOM state is important even if the
        # DOM body doesn't change, for example:
        #
        # 1. Page has 2 checkboxes
        # 2. Checking one checkbox does nothing.
        # 3. Checking both provides data of interest.
        #
        # If we were only counting on the body, then we'd never get to a state
