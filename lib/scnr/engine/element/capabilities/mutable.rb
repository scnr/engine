...
module Mutable
    ...
    module Format
      ...
      # (If no default value exists SCNR::Engine will choose one.)
      APPEND    = 1 << 1
    ...
    EXTRA_NAME      = 'extra_scnr_engine_input'
    FUZZ_NAME       = 'Parameter name fuzzing'
    FUZZ_NAME_VALUE = 'scnr_engine_name_fuzz'
    ...
    def mutable_id
        SCNR::Engine::Element::Capabilities::Mutable.mutable_id(
            method,
            inputs,
            raw_inputs
        )
    ...
    def self.mutable_id( method, inputs, raw_inputs )
        "#{method}:#{SCNR::Engine::Element::Capabilities::Inputtable.inputtable_id(
            inputs, raw_inputs
        )}"
    end
    ...
    def yield_if_unique( element, list )
        ...
        # The auditor may not be interested in this mutation.
        if auditor && auditor.skip?( element )
            print_debug_level_2 "Matched auditor's #skip? method."
            return
        end
