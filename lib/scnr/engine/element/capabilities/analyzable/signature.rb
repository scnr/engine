...
module Signature
    ...
    SIGNATURE_CACHE   = {
        match: Support::Cache::LeastRecentlyPushed.new( size: 10_000 )
    }
    ...
    FILE_SIGNATURES = {
        'environ'  => proc do |response|
            next if !response.body.optimized_include?( 'DOCUMENT_ROOT=' )
            /DOCUMENT_ROOT=.*HTTP_USER_AGENT=/
        end,
        'passwd'   => proc do |response|
            if response.body.optimized_include?( 'bin/' )

                /:.+:\d+:\d+:.+:[0-9a-zA-Z\/]+/

            # Response may have encoded chars as HTML entities.
            elsif response.body.optimized_include?( 'bin&#x2f;' ) &&
                response.body.optimized_include?( '&#x3a;' )
    ...
    def self.calculate_cost( payloads_size, options = {} )
    ...
    def find_substring_and_log( substring, response, opts )
        ...
        SIGNATURE_CACHE[:match][k] = includes = response.body.optimized_include?( substring )
