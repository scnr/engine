...
class NestedCookie < Base
    ...
    class <<self
        ...
        def parse_http_cookie( http_cookie )
            http_cookie.to_s.split( ';' ).inject( {} ) do |h, pair|
                k, v = pair.split( '=', 2 )
                h[decode( k.to_s )] = parse_inputs( v )
                h
            end
        end
...
SCNR::Engine::NestedCookie = SCNR::Engine::Element::NestedCookie
