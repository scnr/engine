...
class Server < Base
    ...
    # {HTTP::Client::Soft404} but this is here to save our ass in case
    ...
    def log_remote_file_if_exists( url, silent = false, options = {}, &block )
        ...
        remote_file_exist?( url ) do |bool, response|
            @_after_run ||= http.after_run( &method(:analyze) )
    ...
    def remote_file_exist?( url, &block )
        ...
        # We need to first train (which performs lots of requests) and
        # nest the request for the response we want to check, rather than
        # the other way around, because nested requests can exceed the max
        # queue size restriction.
        #
        # We may still exceed it, but the overflow will be much smaller
        # this way.
        if http.soft_404.hard?( url )
            http.request( url,
                          method:          :head,
                          performer:       self,
                          fingerprint:     false,
    ...
    def self.flag_issues_as_untrusted( issue_digests )
        issue_digests.uniq.each do |digest|
            next if !(issue = SCNR::Engine::Data.issues[digest])
