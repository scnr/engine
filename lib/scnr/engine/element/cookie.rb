...
class Cookie < Base
    ...
    def initialize( options )
        ...
        # Sanitization for cookies to appear the same from all sources.
        if @data[:path] != '/' && @data[:path].end_with?( '/' )
    ...
    class <<self
        ...
        # Extracts cookies from an HTTP {SCNR::Engine::HTTP::Response response}.
        #
        ...
        def from_response( response )
            from_parser( SCNR::Engine::Parser.new( response ) ) +
        ...
        def from_parser( parser )
            ...
            SCNR::Engine::Utilities.exception_jail {
                e = []
                parser.document.nodes_by_name( :meta ) do |elem|
                    next if elem['http-equiv'].downcase != 'set-cookie'

                    e << from_set_cookie( parser.url, elem['content'] )
                end
                e.flatten.compact
            } rescue []
        end
        ...
        def from_set_cookie( url, str )
            WEBrick::Cookie.parse_set_cookies( str ).flatten.uniq.map do |cookie|
                ...
                if cookie_hash['domain']
                    ...
                    if !SCNR::Engine::URI( "http://#{cookie_hash['domain']}/" ).ip_address?
                        cookie_hash['domain'] =
...
SCNR::Engine::Cookie = SCNR::Engine::Element::Cookie
