...
class Form < Base
    ...
    class <<self
        ...
        def from_response( response, ignore_scope = false )
            from_parser( SCNR::Engine::Parser.new( response ), ignore_scope )
        end
        ...
        def from_parser( parser, ignore_scope = false )
            ...
            e = []
            parser.document.nodes_by_name( :form ) do |node|
        ...
        def from_node( url, node, ignore_scope = false )
            ...
            if (parsed_url = SCNR::Engine::URI( options[:action] ))
            ...
            # However, each SCNR::Engine Form can have only unique input names, so
            ...
            %w(textarea input select button).each do |tag|
                ...
                node.nodes_by_name( tag ) do |elem|
                    ...
                    has_children = false
                    elem.nodes_by_name( 'option' ) do |child|
                        has_children = true
        ...
        # @return   [String]
        def decode( string )
            SCNR::Engine::URI.decode( string.to_s )
        end
...
SCNR::Engine::Form = SCNR::Engine::Element::Form
